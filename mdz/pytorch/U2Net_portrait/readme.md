...
- 0_U2Net_portrait    >模型原始工程，需要自行下载
...
- branch：master
- commit_id：53dc9da
- weights：https://drive.google.com/file/d/1-Yg0cxgrNhHP-016FPdp902BR-kSsA4P/view

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://drive.google.com/file/d/1-Yg0cxgrNhHP-016FPdp902BR-kSsA4P/view ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_U2Net_portrait

```shell
# 在此模型根目录
mkdir 0_U2Net_portrait
git clone -b master https://github.com/xuebinqin/U-2-Net.git 0_U2Net_portrait
cd 0_U2Net_portrait
...
- 1_save.py                              >保存模型，保存好的用于Icraft编译器的模型，会存放在 `/2_compile/fmodel`
  ...
  <div style="background-color: #FFFFCC; color: #000000; padding: 10px; border-left: 5px solid #FFA500;">
  保存模型时的修改点：<br>
  1. 更改多输出为单输出，模型前向只输出F.sigmoid(d0) <br>
  <div>
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/U2Net_portrait_8.toml
     ```
  
      如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）
  
      其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/U2Net_portrait/imodel
  
# 3. 仿真

通过配置3_deploy/modelzoo/U2Net_portrait/cfg/中yaml文件的sim字段为True实现模型仿真。
```json
imodel:
   ...
   sim: true
...
**编译c++程序**
   ...
   ```shell
   #在3.1所需的linux编译环境中
   cd 3_deploy/modelzoo/U2Net_portrait/build_arm
   cmake ..
   make -j
   ```
...
5. **执行程序**
   ...
   ```
   cd /home/fmsh/ModelZoo/modelzoo_v3.0.1/U2Net_portrait/build_arm
   chmod 777 *
   ./U2Net_portrait_psin ../cfg/U2Net_portrait.yaml
   ```
...
| U2Net_portrait | input shape     | hard time      | qt_strategy     | 精度             |
| -------------- | --------------- | -------------- | --------------- | ---------------- |
| float          | [1, 3, 512, 512] | -              | -              | -    |
| 8bit           | [1, 3, 512, 512] | 95.6711ms | kld-pc | - |
| 16bit          | [1, 3, 512, 512] | 186.7045ms | kld-pc | - |
