...
- commit_id：69e1cef
- weights：https://download.fdwxhb.com/data/04_FMSH-100AI/100AI/04_modelzoo/modelzoo_pub/weights/pytorch/CAM++/CAMPPlus_Fbank/best_model/model.pth

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://download.fdwxhb.com/data/04_FMSH-100AI/100AI/04_modelzoo/modelzoo_pub/weights/pytorch/CAM++/CAMPPlus_Fbank/best_model/model.pth ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_CAM++

```shell
# 在此模型根目录
mkdir 0_CAM++
git clone -b release/1.0.5 https://gitee.com/yeyupiaoling/VoiceprintRecognition-Pytorch.git 0_CAM++
cd 0_CAM++
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/CAM++_8.toml
     ```

     如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）

     其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/CAM++/imodel

# 3. 仿真

通过配置3_deploy/modelzoo/CAM++/cfg/中yaml文件的sim字段为true实现模型仿真。
...
2. **执行程序**
   ...
   ```
   python ./CAM++_psin.py ../cfg/CAM++.yaml
   ```
...
* 模型测试使用cn-celeb数据集的test部分，下载测试数据集请存放于  `\3_deploy\modelzoo\CAM++\pyrt\dataset\cn-celeb-test`
...
```
* 运行`0_CAM++/extract_features.py`,给定配置文件后获得处理后的音频输入特征，存放在`cn-celeb-test/enroll_list_features.txt`、`cn-celeb-test/trials_list_features.txt`和`dataset/features`
```
├── features
│   ├── 0
│   ├── 1
...
3. **修改 运行时的yaml配置文件**：
   ...
   运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在 `3_deploy/modelzoo/CAM++/cfg/`

   测试时使用：`CAM++_test.yaml`
...
4. **编译、执行运行时程序**
   ...
   ```
   cd 3_deploy/modelzoo/CAM++/pyrt
   python ./CAM++_test.py ../cfg/CAM++_test.yaml
   ```
...
| CAM++ | input shape     | hard time      | qt_strategy     | 精度             |
| -------------- | --------------- | -------------- | --------------- | ---------------- |
| float          | [1, 365, 80] | -              | -               | <br>EER:0.14227</br> <br>MinDCF:0.66743</br> |
| 8bit           |  [1, 365, 80] | 14.09 ms  | [kld-pc] |<br>EER:0.15173</br> <br>MinDCF:0.71847</br> |
| 16bit          |  [1, 365, 80] | 19.569ms | [kld-pc] |  <br>EER:0.14892</br> <br>MinDCF:0.69324</br>|
