...
class InnerAttention(Module):
    ...
    # revision: 与inner_attention_forward函数中修改后的输入参数保持一致
    def inner_scaled_dot_product_attention(
        ...
        # 去除未输入的attn_mask相关内容（原attn_mask始终为None，无需进入该if分支）
        # if attn_mask is not None:
        #     attn += attn_mask
        attn = softmax(attn, dim=-1)
        # revision: 推理时已去除的原输入dropout_p始终为0.0，无需进入该if分支
        # if dropout_p > 0.0:
    ...
    # revision: 与forward函数中修改后的输入参数保持一致
    def inner_attention_forward(
        ...
        # revision: 精简if/else分支语句
        # if isinstance(embed_dim, torch.Tensor):
        #     # Embed_dim can be a tensor when JIT tracing
        #     head_dim = embed_dim.div(num_heads, rounding_mode='trunc')
        # else:
        # revision: num_heads改为直接读取self.num_heads，而非另行输入
        ...
        # revision: 去除未输入的attn_mask, key_padding_mask参数相关内容（后续过程中未使用该参数）
        # Prep attention mask
        # if attn_mask is not None:
        ...
        # revision: 
        # 1. 精简if/else分支语句
        # 2. num_heads改为直接读取self.num_heads，而非另行输入
        ...
        #     k = static_k
        # if static_v is None:
        v = v.contiguous().view(-1, bsz * self.num_heads, self.vdim // self.num_heads).transpose(0, 1)
        # else:
        #     v = static_v
        ...
        # revision:
        # 1. 去除输入参数中的attn_mask，后续过程中未使用该参数
        # 2. 去除输入参数中的dropout_p，为非Tensor类型常量，改为根据该值直接在后续函数中进行相应修改
        ...
        attn_output, attn_output_weights = self.inner_scaled_dot_product_attention(q, k, v)
    ...
    def forward(self, query: Tensor, key: Tensor, value: Tensor) -> Tuple[Tensor, Optional[Tensor]]:
        ...
        # revision: 去除输入参数中的未使用项和其它非Tensor类型项
        attn_output, attn_output_weights = self.inner_attention_forward(
            query, key, value,
            # self.embed_dim, self.num_heads,
            self.q_proj_bias, self.k_proj_bias,
...
class CorrAttention(Module):
    ...
    def forward(self, corr_map, pos_emb):
        # revision for icraft:
        # 1. 对pos_emb的torch.repeat_interleave操作移至网络外作为前处理计算
        # 2. 修改后输入的pos_emb与原本的shape不同，按原shape值指定batch_size
        batch_size = 1  # pos_emb.shape[1]
...
class AiAModule(Module):
    ...
    # revision: 与aia_attention_forward函数中修改后的输入参数保持一致
    ...
    def aia_attention_forward(
        ...
        #                                                                          pos_emb)
        attn_output, attn_output_weights = self.aia_scaled_dot_product_attention(q, k, v, pos_emb)
    ...
    def forward(self, query: Tensor, key: Tensor, value: Tensor,
        ...
        # else:
        # revision: 去除输入参数中的key_padding_mask和其它非Tensor类型项、未使用项
        attn_output, attn_output_weights = self.aia_attention_forward(
            query, key, value,  # self.embed_dim, self.num_heads,
            self.in_proj_weight, self.in_proj_bias,
            # self.bias_k, self.bias_v, self.add_zero_attn,
