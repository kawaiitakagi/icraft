...
- branch：main
- commit_id：cdb3a4e
- weights：https://github.com/Little-Podi/AiATrack/releases/download/downloads/Trained_Models.zip

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://github.com/Little-Podi/AiATrack/releases/download/downloads/Trained_Models.zip ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_AiATrack

```shell
# 在此模型根目录
mkdir 0_AiATrack
git clone -b main https://github.com/Little-Podi/AiATrack 0_AiATrack
cd 0_AiATrack
git checkout cdb3a4e
```

3）安装好AiATrack源码readme要求的环境后，将1_scripts中文件复制到0_ETTrack中，执行保存模型脚本1_save.py
...
保存模型时的修改点：
- backbone
  - 去除backbone embeding前处理计算部分，仅保留resnet50模型结果
  - 将`norm_layer`由***FrozenBatchNorm2d***改为***nn.BatchNorm2d***；
  - 将*backbone*(*Joiner*类)及其下*BackboneBase*类的输入和输出格式统一改为`Tensor`;
  - 去除*BackboneBase*中的`mask`相关内容，移至*网络外*作为前处理计算
  - 去除以*backbone*输出为输入的*adjust*函数中的`mask`, `pos_embed`, `inr_embed`相关内容，移至*网络外*作为前处理计算，并将其输入和输出格式统一改为`Tensor`。

- transformer
  - 去除*TransformerEncoder*和*TransformerEncoderLayer*输入参数中的`src_key_padding_mask`（实际未使用）；
  - 去除*TransformerDecoder*和*TransformerDecoderLayer*输入参数中的`refer_msk_list`（实际未使用）；
  - 将*TransformerDecoderLayer*中对`refer_mem_list`, `refer_emb_list`, `refer_pos_list`的`torch.cat`操作移至*网络外*作为前处理计算；
  - 将*AiAModule.CorrAttention*中对`pos_emb`的`torch.repeat_interleave`操作移至*网络外*作为前处理计算；
  - 精简*AiAModule.forward*中一些未使用的输入参数和`if`/`else`分支语句。

- box_head
  - 在***init***函数中新增一个`shape`为`(feat_size*feat_size, 1)`、值均为*1.0*的`float`类型`Tensor`；
  - 将*soft_argmax*函数中的`torch.sum`函数改为与上述新增的常量`Tensor`进行`torch.matmul`操作；
  - 将*forward*函数中的`torch.stack`函数改为`torch.cat`和`reshape`;
  - 将以*box_head*输出为输入的*box_xyxy_to_xywh*, *box_xyxy_to_cxcywh*函数计算移至*网络外*作为后处理计算。
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/AiATrack_8.toml
     ```

     如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）

     其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/AiATrack/imodel

# 3. 仿真

通过配置3_deploy/modelzoo/AiATrack/cfg/中yaml文件的sim字段为True实现模型仿真。
...
2. **执行程序**
   ...
   ```
   python ./AiATrack_psin.py ../cfg/AiATrack.yaml
   ```
...
- 模型测试使用LaSOT数据集中随机抽取的部分数据集，测试序列存放于  `3_deploy\modelzoo\AiATrack\io\input`及 `3_deploy\modelzoo\AiATrack\io\test.txt`中
- 如需测试其他数据集，需自行将图片存放于 `3_deploy\modelzoo\AiATrack\io\input`,并准备对应的input.txt

2.**修改 运行时的yaml配置文件**：

   运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在 `3_deploy/modelzoo/AiATrack/cfg/`

   测试时使用：`AiATrack_psin.yaml`
...
```
cd 3_deploy/modelzoo/AiATrack/pyrt
python ./AiATrack_psin.py ../cfg/AiATrack_test.yaml
```
...
| AiATrack | input shape      | hard time       |
| --------------- | ---------------- | --------------- |
| float           | [1, 320, 320, 3] | -               |
| int8            | [1, 320, 320, 3] | 91.605ms |
| int16           | [1, 320, 320, 3] | 183.452ms |

精度测试结果如下：

| AiATrack | qt_strategy | AUC   | Precision | Norm Precision |
| -------- | ----------- | ----- | --------- | -------------- |
| float    | -           | 65.74 | 71.69     | 76.23          |
| 8bit     | null-pc     | 58.72 | 61.62     | 69.96          |
| 16bit    | null-pc     | 59.08 | 63.69     | 69.64          |