...
- code：https://github.com/yjn870/RDN-pytorch.git
- branch：master
- commit_id：f2641c0
- weights：https://www.dropbox.com/s/pd52pkmaik1ri0h/rdn_x2.pth?dl=0

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://www.dropbox.com/s/pd52pkmaik1ri0h/rdn_x2.pth?dl=0 ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_rdn

```shell
# 在此模型根目录
mkdir 0_rdn
git clone -b master https://github.com/yjn870/RDN-pytorch.git 0_rdn
cd 0_rdn
git checkout f2641c0
```
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/rdn_2x_8.toml
     ```

     如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）

     其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/rdn/imodel

# 3. 仿真

通过配置3_deploy/modelzoo/rdn/cfg/中yaml文件的sim字段为True实现模型仿真。
...
2. **执行程序**
   ...
   ```
   python ./rdn_psin.py ../cfg/rdn_2x.yaml
   ```
...
1. **数据集准备**
   ...
   - 测试数据集下载：[Set5eval](https://www.dropbox.com/s/b7v5vis8duh9vwd/Set5_x2.h5?dl=0)
   - 自行将图片存放于 `3_deploy\modelzoo\yolov5_7.0\io\input`,并准备对应的input.txt
...
2. **执行运行时程序**
   ...
   - python运行时
     ...
     ```
     cd 3_deploy/modelzoo/rdn/pyrt
     python ./rdn_psin.py ../cfg/rdn_2x.yaml
     ```

     直接运行脚本 每张图片会输出自己的psnr值，set5val一共张图片，5个psnr值求得平均值即为测得精度

# 6. 模型性能记录

| rdn | input shape      | hard time       | qt_strategy      | 精度              |
| --------------- | ---------------- | --------------- | ---------------- | ----------------- |
| float           | [1, 3, 256, 256] | -               | -                | PSNR:44.668 |
| parse | [1, 3, 256, 256] |  |  | PSNR:44.668 |
| int8            | [1, 3, 256, 256] | 928.4067ms  | kld-pc | PSNR:42.656 |
| int16           | [1, 3, 256, 256] | 1904.2732ms | kld-pc      | PSNR:43.172 |
