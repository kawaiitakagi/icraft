...
void map_plot(const cv::Mat& src_mat, cv::Mat& img_id_mat, cv::Mat& img_color_mat, const cv::Mat& dst_img, cv::Mat& res_color_mat) {
    ...
    for (int row = 0; row < src_mat.rows; row++) {
        ...
        for (int col = 0; col < src_mat.cols; col++) {
            if(channels == 1) {
                int8_t value = src_mat.at<int8_t>(row, col); // 获取像素值
...
void post_process_seg(const std::vector<Tensor>& result_tensor, PicPre& img, NetInfo& netinfo, bool& show, bool& save, std::string& resRoot, std::string& name
    ...
    cv::Mat net_result_mat;
    auto output_channel = result_tensor[0].dtype()->shape[3] == 1;
    auto host_tensor = result_tensor[0].to(HostDevice::MemRegion());

    if (output_channel == 1) { // 使用了segpost，输出单通道
        auto tensor_data = (int8_t*)host_tensor.data().cptr();//获取结果的指针
        net_result_mat = cv::Mat(result_tensor[0].dtype()->shape[1], result_tensor[0].dtype()->shape[2], CV_8SC(result_tensor[0].dtype()->shape[3]), tensor_data);
    }
    else {
        auto tensor_data = (float*)host_tensor.data().cptr();
        net_result_mat = cv::Mat(result_tensor[0].dtype()->shape[1], result_tensor[0].dtype()->shape[2], CV_32FC(result_tensor[0].dtype()->shape[3]), tensor_data);
    }
    auto h = netinfo.i_cubic[0].h;
    auto w = netinfo.i_cubic[0].w;
    cv::Mat resultMat;
    if (output_channel == 1) { // 使用了segpost，输出单通道
        resultMat = net_result_mat;
    }
    else {
