...
- branch：master
- commit_id：be1a81a
- weights：https://drive.google.com/file/d/16viDZhbmuc3y7OSsUo2vhA7V6kYO0KX6/view

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://drive.google.com/file/d/16viDZhbmuc3y7OSsUo2vhA7V6kYO0KX6/view ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_DDRNet

```shell
# 在此模型根目录
mkdir 0_DDRNet
git clone -b master https://gitee.com/mxh-spiger/custom_model.git 0_DDRNet
cd 0_DDRNet
git checkout be1a81a
```

3）手动将0_DDRNet/DDRNet中的所有文件复制到0_DDRNet中，并将1_scripts中的所有文件复制到0_DDRNet中，并在0_DDRNet文件夹下执行保存模型脚本
...
- 1_save.py                              >保存模型，保存好的用于Icraft编译器的模型，会存放在 `/2_compile/fmodel`
  ...
  <div style="background-color: #FFFFCC; color: #000000; padding: 10px; border-left: 5px solid #FFA500;">
  保存模型时的修改点：<br>
  1. 去除模型子分支，确保只有一个输出，即将DualResNet的augment参数设置为False;<br>
  2. 由于icraft不支持除法取整‘//’，引入全局变量OUT_W和OUT_H，提前将width_output和height_output结果计算出来，并将其前向函数width_output和height_output替换为int类型的全局变量；<br>
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/DDRNet_8.toml
     ```
  
      如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）
  
      其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/DDRNet/imodel
  
# 3. 仿真

通过配置3_deploy/modelzoo/DDRNet/cfg/中yaml文件的sim字段为True实现模型仿真。
```json
imodel:
   ...
   sim: true
...
**编译c++程序**
   ...
   ```shell
   #在3.1所需的linux编译环境中
   cd 3_deploy/modelzoo/DDRNet/build_arm
   cmake ..
   make -j
   ```
...
5. **执行程序**
   ...
   ```
   cd /home/fmsh/ModelZoo/modelzoo/DDRNet/build_arm
   chmod 777 *
   ./DDRNet_psin ../cfg/DDRNet.yaml
   ```
...
1. **数据集准备**
   ...
   测试数据集下载：[cityscapes](https://www.cityscapes-dataset.com/)

   * 模型测试使用Cityscape数据集，提前下载并将测试图片存放于  `3_deploy\modelzoo\DDRNet\io\cityscapes\leftImg8bit`及`3_deploy\modelzoo\DDRNet\io\cityscapes\valImages.txt`中
   * 如需测试其他数据集，需自行将图片存放于`3_deploy\modelzoo\DDRNet\io\input`,并准备对应的input.txt



2. **修改 运行时的yaml配置文件**：

   运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在`3_deploy/modelzoo/DDRNet/cfg/`

   测试时使用：`DDRNet_test.yaml`
...
3. **编译、执行运行时程序**
   ...
   ```shell
   cd 3_deploy/modelzoo/DDRNet
   mkdir build_win
   cd build_win
   cmake ..
   cmake --build . --config Release
   ./Release/DDRNet_psin.exe ../cfg/DDRNet_test.yaml
   ```

   注意：如果使用visualstudio打开sln进行调试，需要右键工程-属性-调试-命令参数中配置` ../cfg/DDRNet_test.yaml`
...
4. **使用metrics测试脚本，将保存的测试结果计算出精度指标**
   ...
   - 本模型使用`./semantic-seg_cityscapes/test.py` 
   - 需要手动修改config.yaml中的路径：
   - results_root：测试结果保存的路径



# 6. 模型性能记录

| DDRNet | input shape     | hard time      | qt_strategy     | 精度             |
| -------------- | --------------- | -------------- | --------------- | ---------------- |
| float          | [1, 3, 1024, 2048] | -              | -               | mIoU:79.51 |
| 8bit           | [1, 3, 1024, 2048] | 75.0438ms | kld-pc | mIoU:79.27 |
| 16bit          | [1, 3, 1024, 2048] | 161.9397ms | kld-pc | mIoU:79.40 |
