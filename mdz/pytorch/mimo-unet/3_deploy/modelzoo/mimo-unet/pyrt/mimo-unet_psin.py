...
if __name__ == "__main__":
    # 获取yaml
    Yaml_Path = "../cfg/mimo-unet_test.yaml"
    if len(sys.argv) < 2:
    ...
    for line in tqdm(open(imgList, "r")):
        ...
        # img_=np.array(img).astype(np.float32).reshape(1, netinfo.i_cubic[0].h, netinfo.i_cubic[0].w, 3) 
        ...
        # file = open("no_imk.ftmp", 'wb')  # 使用二进制
        # output_tensors[0].dump(file,"SFB")
        ...
        gen_img = np.array(output_tensors[2]).astype(np.float32)
        gen_img = np.squeeze(gen_img, axis=0)
        gen_img = (gen_img*255).round()
        # gen_img = cv2.cvtColor(gen_img,cv2.COLOR_RGB2BGR)
        output_path = img_path.replace('input','output').replace('.png','_result.png')
