...
- branch：master
- commit_id：5c58013
- weights：https://drive.google.com/file/d/1EQJoQj3YMLFfzrbgzWMD3Xj96RqLdIlx/view?usp=sharing

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://drive.google.com/file/d/1EQJoQj3YMLFfzrbgzWMD3Xj96RqLdIlx/viewusp=sharing ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_mimo-unet

```shell
# 在此模型根目录
mkdir 0_mimo-unet
git clone -b master https://github.com/chosj95/MIMO-UNet 0_mimo-unet
cd 0_mimo-unet
git checkout 5c58013
```
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/mimo-unet.toml
     ```

     如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）

     其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/mimo-unet/imodel

# 3. 仿真

通过配置3_deploy/modelzoo/mimo-unet/cfg/中yaml文件的sim字段为True实现模型仿真。
...
1. **python运行虚拟环境要求与准备**
   ...
   - 在选定的python运行虚拟环境中安装python运行时所需要的依赖包
     ...
     ```
     cd 3_deploy/modelzoo/mimo-unet
...
2. **执行程序**
   ...
   ```
   python ./mimo-unet_psin.py ../cfg/mimo-unet.yaml
   ```
...
* 模型测试使用GoPro，测试图片存放于  `3_deploy\modelzoo\mimo-unet\io\input`及 `3_deploy\modelzoo\mimo-unet\io\list.txt`中
* 如需测试其他数据集，需自行将图片存放于 `3_deploy\modelzoo\mimo-unet\io\input`,并准备对应的input.txt

3. **修改 运行时的yaml配置文件**：

   运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在 `3_deploy/modelzoo/mimo-unet/cfg/`

   测试时使用：`mimo-unet_test.yaml`
...
```
cd 3_deploy/modelzoo/mimo-unet/pyrt
python ./mimo-unet_psin.py ../cfg/mimo-unet_test.yaml
```
...
| mimo-unet       | input shape       | hard time       | qt_strategy      | 精度              |
| --------------- | ----------------  | --------------- | ---------------- | ----------------- |
| float           | [1, 3, 720, 1280] | -               | -                | psnr:31.891 |
| int8            | [1, 3, 720, 1280] | 759ms           | kld-pc           | -                 |
| int16           | [1, 3, 720, 1280] | 1459ms          | kld-pc           | -                 |
