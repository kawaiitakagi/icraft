...
1）根据模型来源中的地址：https://github.com/ultralytics/yolov5/releases/download/v7.0/yolov5s.pt ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_yolov5_7.0

```shell
# 在此模型根目录
mkdir 0_yolov5_7.0
git clone -b master https://github.com/ultralytics/yolov5.git 0_yolov5_7.0
cd 0_yolov5_7.0
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```
      ...
      其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/yolov5_7.0/imodel
  
# 3. 仿真

通过配置3_deploy/modelzoo/yolov5_7.0/cfg/中yaml文件的sim字段为True实现模型仿真。
```json
imodel:
   ...
   sim: true
...
2. **执行程序**
   ...
   ```
   python ./yolov5_psin.py ../cfg/yolov5s.yaml
   ```
...
**编译c++程序**
   ...
   ```shell
   #在3.1所需的linux编译环境中
   cd 3_deploy/modelzoo/yolov5_7.0/build_arm
   cmake ..
   make -j
   ```
...
5. **执行程序**
   ...
   ```
   cd /home/fmsh/ModelZoo/modelzoo/yolov5_7.0/build_arm
   chmod 777 *
   ./yolov5s_psin ../cfg/yolov5s.yaml
   ```
...
3. **修改 运行时的yaml配置文件**：
   ...
   运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在`3_deploy/modelzoo/yolov5_7.0/cfg/`

   测试时使用：`yolov5s_test.yaml`
...
./Release/yolov5s.exe ../cfg/yolov5s_test.yaml
```

注意：如果使用visualstudio打开sln进行调试，需要右键工程-属性-调试-环境中配置`./Release/yolov5s.exe ../cfg/yolov5s_test.yaml`
...
| yolov5_7.0 | input shape     | hard time      | qt_strategy     | 精度             |
| -------------- | --------------- | -------------- | --------------- | ---------------- |
| float          | [1, 3, 640, 640] | -              | -               | ap50:56.8; ap50-95:37.4 |
| int8           | [1, 3, 640, 640] | 5.8499ms | kld-pc | ap50:55.1; ap50-95:34.7 |
| int16          | [1, 3, 640, 640] | 13.3739ms | kld-pc | ap50:56.2; ap50-95:37.1 |
