...
- code：https://github.com/Megvii-BaseDetection/YOLOX.git
- branch：main
- commit_id：ac58e0a
...
1）根据模型来源中的地址：https://github.com/Megvii-BaseDetection/YOLOX/releases/download/0.1.1rc0/yolox_s.pth ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码并安装yolox，文件夹名称要设置为：0_yolox

```shell
# 在此模型根目录
mkdir 0_yolox
git clone -b main https://github.com/Megvii-BaseDetection/YOLOX.git 0_yolox
cd 0_yolox
...
- 1_save.py                              >保存模型，保存好的用于Icraft编译器的模型，会存放在 `/2_compile/fmodel`
  ...
  <div style="background-color: #FFFFCC; color: #000000; padding: 10px; border-left: 5px solid #FFA500;">
  保存模型时的修改点：<br>
  1. 将Focus类内的切片操作用卷积等效替换，将左上，左下，右上，右下处进行通道维拼接<br>
  2. 将yolo_head前向函数中的output按照obj、reg、cls的顺序输出<br>
  </div>
...
- 3）**执行编译：**
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/yolox_s_8.toml
     ```
  

​	如果过程顺利，将在imodel中得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）
​	其中包括编译各阶段产生的中间结果模型，最终用于片上部署的模型直接被保存到:  3_deploy/modelzoo/yolox/imodel

  

# 3. 仿真

通过配置3_deploy/modelzoo/yolox/cfg/中yaml文件的sim字段为True实现模型仿真。
...
1. **编译环境准备**
   ...
   **编译c++程序**
   ...
   ```shell
   #在3.1所需的linux编译环境中
   cd 3_deploy/modelzoo/yolox/build_arm
   cmake ..
   make -j
   ```
...
5.**执行程序**
   ...
   ```
   cd /home/fmsh/ModelZoo/modelzoo/yolox/build_arm
   chmod 777 *
   ./yolox_s_psin ../cfg/yolox_s.yaml
   ```
...
`yolox_s_plin.cpp`：该脚本是bytetrack多目标追踪的多线程PLIN demo。此demo将网络拆分为三部分（imk、icore+detpost、后处理）多线程并行加速；

**使用方法**：

​		a）首先，需要重新编译模型，包括：`yolox_s_plin.toml`;

​		b）修改cmakelist.txt，将第三行修改为set(TARGET_NAME yolox_s_plin)，并按照章节4.3重新编译c++程序，指令如下：
...
cd /home/fmsh/ModelZoo/modelzoo/yolox/build_arm
chmod 777 *
./yolox_s_plin ../cfg/yolox_s_plin.yaml
```
...
1. **如果使用了DetPost硬算子**（如果未使用则忽略此条）
   ...
   - 1）修改customop toml
     ...
     customop toml 即 config\customop\yolox.toml
...
2. **数据集准备**
   ...
   测试数据集下载：[COCOval2017](http://images.cocodataset.org/zips/val2017.zip)

   - 模型测试使用COCOval2017数据集，测试图片存放于  `3_deploy\modelzoo_v2.2.0\yolox\io\coco`文件夹下，对应的list文件存放于`3_deploy\modelzoo_v2.2.0\yolox\io\coco\cocoval_2017.txt`中

   - 如需测试其他数据集，需自行将图片存放于`3_deploy\modelzoo_v2.2.0\yolox\io\input`,并准备对应的input.txt

3. **修改运行时的yaml配置文件**：

   运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在`3_deploy/modelzoo/yolox/cfg/`

   测试时使用：`yolox_s_test.yaml`中的参数设置

   需要修改：

   - imodel中参数：设置imodel路径，并根据所使用板子ip设置ip，如需保存图片将save设置为true，如需可视化将show设置为true，反之，设置为false；
   - dataset中参数：设置dataset精度测试数据集路径；
   - param中参数：测精度时需设置conf为0.001。
...
4. **编译、执行运行时程序**
   ...
   ```shell
   cd 3_deploy/modelzoo/yolox
   mkdir build_win
   cd build_win
   cmake ..
   cmake --build . --config Release
   ./Release/yolox_s_psin.exe ../cfg/yolox_s_test.yaml
   ```

   注意：如果使用visualstudio打开sln进行调试，需要右键工程-属性-调试-命令参数中配置` ../cfg/yolox_s_test.yaml`
...
5. **使用metrics测试脚本，将保存的测试结果计算出精度指标**
   ...
   本模型使用`./box_mAP/calAP.py`，需要手动修改以下路径：

   - originImagesDir：测试集图片的路径
   - inferenceOutputDir：测试结果保存的路径
   - dtJsonPath：精度测试结果保存路径



# 5. 模型性能记录

| yolox | input shape | hard time | qt_strategy | 精度                 |
| -------------- | ----------- | --------- | ----------- | -------------------- |
| float          | [1, 3, 640, 640] | -         | -           | ap50-95:40.5 |
| parse | [1, 3, 640, 640] | - | - | ap50:59.6 ap50-95:40.7 |
| 8bit        | [1, 3, 640, 640] | 7.4136ms | kld-pt | ap50:58.6; ap50-95:38.5 |
| 16bit       | [1, 3, 640, 640] | 16.7926ms | kld-pt | ap50:59.5; ap50-95:40.2 |
