...
- branch：master
- commit_id：acba757
- weights：https://github.com/The-Learning-And-Vision-Atelier-LAVA/PASSRnet/blob/master/log/x4/PASSRnet_x4.pth

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）下载源代码时，权重文件会直接下载，并存放于./log/x4中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_PASSRnet

```shell
# 在此模型根目录
mkdir 0_PASSRnet
git clone -b master https://github.com/The-Learning-And-Vision-Atelier-LAVA/PASSRnet 0_PASSRnet
cd 0_PASSRnet
git checkout acba757
```
...
- 1_save.py                              >保存模型，保存好的用于Icraft编译器的模型，会存放在 `/2_compile/fmodel`
  ...
  <div style="background-color: #FFFFCC; color: #000000; padding: 10px; border-left: 5px solid #FFA500;">
  保存模型时的修改点： <br>
    1.去除foward函数中的判断条件，移除is_training=0条件分支 <br>
      2.只返回上采样后的输出out <br>
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/PASSRnet_8.toml
     ```

     如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）

     其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/PASSRnet/imodel

# 3. 仿真

通过配置3_deploy/modelzoo/PASSRnet/cfg/中yaml文件的sim字段为True实现模型仿真。
...
2. **执行程序**
   ...
   ```
   python ./PASSRnet_psin.py ../cfg/PASSRnet.yaml
   ```
...
| PASSRnet | input shape      | hard time       | qt_strategy      | 精度              |
| --------------- | ---------------- | --------------- | ---------------- | ----------------- |
| float           | [1, 125, 178, 3] | -               | -                | - |
| int8            | [1, 125, 178, 3] | 61.0188ms  | null-pt | -  |
| int16           | [1, 125, 178, 3] | 164.7030ms | null-pt | - |
