...
- code：https://github.com/hukaixuan19970627/yolov5_obb.git
- branch：master
- commit_id：b00c3f2
- weights：https://download.fdwxhb.com/data/04_FMSH-100AI/100AI/04_modelzoo/modelzoo_pub/weights/pytorch/yolov5_obb/best.pt

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://download.fdwxhb.com/data/04_FMSH-100AI/100AI/04_modelzoo/modelzoo_pub/weights/pytorch/yolov5_obb/best.pt ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_yolov5_obb

```shell
# 在此模型根目录
mkdir 0_yolov5_obb
git clone -b master https://github.com/hukaixuan19970627/yolov5_obb.git 0_yolov5_obb
cd 0_yolov5_obb
git checkout b00c3f2
```
执行脚本前，请在0_yolov5_obb下进行环境设置。
```shell
...
python 1_save.py
```
注：环境搭建及依赖请参考`1_scripts/yolov5_obb-install.txt`
...
- 0_infer.py                      	    >可以推理一张图并得到最终结果，模型原始权重会从 `/weights	`中寻找，需要您预先下载<br>
 注：环境搭建及依赖请参考`1_scripts/yolov5_obb-install.txt`<br>
...
- 1_save.py                              >保存模型，保存好的用于Icraft编译器的模型，会存放在 `/2_compile/fmodel`
  ...
  1. 将Detect forward中conv算子之后的后处理操作去掉<br>
  2. 定义new_forward对forward进行替换<br>
     </div>
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/yolov5m_obb_8.toml
     ```
  
      如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）
  
      其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/yolov5_obb/imodel
  
# 3. 仿真

通过配置3_deploy/modelzoo/yolov5_obb/cfg/中yaml文件的sim字段为True实现模型仿真。
```json
imodel:
   ...
   sim: true
...
**编译c++程序**
   ...
   ```shell
   #在3.1所需的linux编译环境中
   cd 3_deploy/modelzoo/yolov5_obb/build_arm
   cmake ..
   make -j
   ```
...
5. **执行程序**
   ...
   ```
   cd /home/fmsh/ModelZoo/modelzoo/yolov5_obb/build_arm
   chmod 777 *
   ./yolov5_obb ../cfg/yolov5_obb.yaml
   ```
...
## 4.2 PLIN工程示例   
本工程还提供了PLIN demo<br>
`yolov5_obb_plin.cpp`：为yolov5_obb目标检测的单线程PLIN demo<br>
使用方法：
* 进入`2_compile` 重新编译模型，编译配置为：`yolov5m_obb_8_plin.toml`
* 进入`3_deploy` 修改CMakeList.txt
```C++
# set sln name
set(TARGET_NAME yolov5_obb_plin)
```
* 编译c++程序，将执行文件拷贝到板子上，程序执行指令如下
```
cd /home/fmsh/ModelZoo/modelzoo/yolov5_obb/build_arm
chmod 777 *
./yolov5_obb_plin ../cfg/yolov5_obb.yaml
```
# 5. 精度测试
...
* 测试数据集下载：[DOTA-v1.5](https://captain-whu.github.io/DOAI2019/dataset.html)

* 模型测试使用DOTA-v1.5数据集的Validation部分，先使用 `0_yolov5_obb/DOTA_devkit/ImgSplit_multi_process.py` 脚本对数据进行预处理（将原图切分为1024*1024大小、gap为200的多张小图），再将切分后的测试图片存放于  `3_deploy\modelzoo\yolov5_obb\io\input`及`3_deploy\modelzoo\yolov5_obb\io\DOTA-v1.5_val.txt`中
* 如需测试其他数据集，需自行将图片存放于`3_deploy\modelzoo\yolov5_obb\io\input`，并准备对应的input.txt



3. **修改 运行时的yaml配置文件**：

   运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在`3_deploy/modelzoo/yolov5_obb/cfg/`

   测试时使用：`yolov5_obb_test.yaml`
...
```shell
cd 3_deploy/modelzoo/yolov5_obb
mkdir build_win
cd build_win
cmake ..
cmake --build . --config Release
./Release/yolov5s.exe ../cfg/yolov5_obb_test.yaml
```

注意：如果使用visualstudio打开sln进行调试，需要右键工程-属性-调试-环境中配置`./Release/yolov5_obb.exe ../cfg/yolov5_obb_test.yaml`



5. **使用metrics测试脚本，将保存的测试结果计算出精度指标**

对预处理后的图片进行metrics测试，metrics测试脚本使用与配置说明：
- 精度测试脚本存放于：<https://gitee.com/mxh-spiger/benchmark> 
- 本模型使用`./obb/calAP_obb.py` 
- resDir字段设置为片上系统预测结果存放路径
- 其它可选配置可参考`./obb/readme.md`



# 6. 模型性能记录

| yolov5_obb | input shape     | hard time      | qt_strategy     | 精度             |
| -------------- | --------------- | -------------- | --------------- | ---------------- |
| float          | [1, 3, 1024, 1024] | -              | -               | ap50: 69.6, ap50-95: 41.72 |
| parsed         | [1, 3, 1024, 1024] | -              | -               | ap50: 68.97, ap50-95: 41.32 |
| 8bit           | [1, 3, 1024, 1024] | 37.98692ms | kld-pc | ap50: 67.05, ap50-95: 39.39 |
| 16bit          | [1, 3, 1024, 1024] | 80.97194ms | kld-pc | ap50: 67.71, ap50-95: 40.73 |
