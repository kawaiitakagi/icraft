...
#include "postprocess_yolov5_obb.hpp"
    ...
    bool cudamode = imodel["cudamode"].as<bool>();
        ...
        while (true) {
            ...
            post_results = post_detpost_plin(icore_tensor, post_results, netinfo, conf, iou_thresh, MULTILABEL, fpga_nms,
                N_CLASS, ANCHORS,device,run_sim,_norm,real_out_channles,_stride,bbox_info_channel,mat,LABELS,ratio_bias);
            ...
            std::vector<std::vector<float>> output_data = std::get<3>(post_results);
            ...
            for (auto res : output_data) {
                ...
                //draw results
                std::vector<std::vector<cv::Point>> point_list = { { cv::Point(point_x1,point_y1),cv::Point(point_x2,point_y2),cv::Point(point_x3,point_y3),cv::Point(point_x4,point_y4) } };
                cv::Scalar color_ = cv::Scalar(u(e), u(e), u(e));
                cv::drawContours(mat, point_list, -1, color_, 2);
                std::stringstream ss;
                ss << std::fixed << std::setprecision(2) << score;
                std::string s = std::to_string(class_id) + "_" + LABELS[class_id] + " " + ss.str();
                auto s_size = cv::getTextSize(s, cv::FONT_HERSHEY_DUPLEX, 0.5, 1, 0);

                cv::rectangle(mat, cv::Point2f(point_x1 - 1, point_y1 - s_size.height - 7), cv::Point2f(point_x1 + s_size.width, point_y1 - 2), color_, -1);
                cv::putText(mat, s, cv::Point2f(point_x1, point_y1 - 2), cv::FONT_HERSHEY_DUPLEX, 0.5, cv::Scalar(255, 255, 255), 0.2);
