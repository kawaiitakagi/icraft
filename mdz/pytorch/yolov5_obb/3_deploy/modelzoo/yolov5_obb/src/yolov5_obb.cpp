...
int main(int argc, char* argv[])
{
	...
	std::vector<int> ori_out_channles = { 3 * (N_CLASS + 180 + 5) };
	...
	for (auto name : namevector) {
		...
		if(netinfo.DetPost_on){
			...
			post_detpost_hard(outputs, img,netinfo, conf, iou_thresh, MULTILABEL, fpga_nms, 
			N_CLASS,ANCHORS,LABELS,show,save,resRoot,name,device,run_sim,_norm,_stride);
		}
		else{
			
			post_detpost_soft(outputs, img,netinfo, conf, iou_thresh, MULTILABEL, fpga_nms, 
			N_CLASS,ANCHORS,LABELS,show,save,resRoot,name,device,run_sim);
		}
