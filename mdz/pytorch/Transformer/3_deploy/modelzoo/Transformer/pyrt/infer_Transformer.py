...
class Predict:
    def __init__(self, model_name='Transformer', dataset='../vocab/', embedding='embedding_SougouNews.npz', use_word=False):
        if use_word:
    ...
    def predict_icraft(self, query):
        ...
        with torch.no_grad():
            ...
            # embedding_out.tofile(R"../2_compile/qtset/Transformer/postion_embedding_out.ftmp")
            ...
            print("Predict data: " ,outputs_icraft.numpy()[0])
...
if __name__ == "__main__":
    ...
    # 获取yaml
    Yaml_Path = "../cfg/Transformer.yaml"
    if len(sys.argv) < 2:
    ...
    with open(resRoot + '/infer_Transformer_result.log', 'w', encoding='utf-8') as f:
        f.write("Input text: " )
        f.write(query+ '\n' )
