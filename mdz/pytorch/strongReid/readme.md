...
- branch：master
- commit_id：3da7e6f
- weights：https://drive.google.com/open?id=1_r4wp14hEMkABVow58Xr4mPg7gvgOMto

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://drive.google.com/open?id=1_r4wp14hEMkABVow58Xr4mPg7gvgOMto ，下载原始weights，存放于 `/weights`文件夹中

该下载链接完成下载后为压缩包，解压缩之后为所需要的market_resnet50_model_120_rank1_945.pth文件
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_strongReid

```shell
# 在此模型根目录
mkdir 0_strongReid
git clone -b master https://github.com/michuanhaohao/reid-strong-baseline.git 0_strongReid
cd 0_strongReid
git checkout 3da7e6f
```
...
- 3）**执行编译：**
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/strongReid_8.ini
     ```
  

​	如果过程顺利，将在imodel中得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）
​	其中包括编译各阶段产生的中间结果模型，最终用于片上部署的模型直接被保存到:  3_deploy/modelzoo/strongReid/imodel
...
通过配置3_deploy/modelzoo/strongReid/cfg/中yaml文件的sim字段为True实现模型仿真。
...
1. **python运行虚拟环境要求与准备**
   ...
   - 在选定的python运行虚拟环境中安装python运行时所需要的依赖包
     ...
     ```powershell
     cd 3_deploy/modelzoo/strongReid
...
2. **执行程序**
   ...
   ```
   python strongReid_psin.py ../cfg/strongReid_psin.yaml
   ```
...
1. **数据集准备**
   ...
   - 测试数据集下载：[Market1501](http://www.liangzheng.org/Project/project_reid.html)
   - Extract dataset The data structure would like:
...
运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在 `3_deploy/modelzoo/strongReid/cfg/`

测试时使用：`strongReid_test.yaml`
...
```
cd 3_deploy/modelzoo/strongReid/pyrt
python strongReid_psin.py ../cfg/strongReid_test.yaml
```

运行完之后，可以在命令窗口看到打印map的精度信息 

# 6. 模型性能记录

|            | input shape   | hard time                                                  | 精度                                                         |
| ---------- | ------------- | ---------------------------------------------------------- | ------------------------------------------------------------ |
| strongReid | [1,3,256,128] | {'8bit': '3.4699 ms', '16bit': '7.2025 ms', 'mixed': None} | {float: [map:85.9],parse: [map:85.9],8bit: [kld-pt,map:83.3],16bit: [kld-pt,map:84.8], mixed: None,} |