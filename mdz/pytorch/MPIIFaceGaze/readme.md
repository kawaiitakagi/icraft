...
- branch：master
- commit_id：2a31a6c
- weights：https://github.com/hysts/pytorch_mpiigaze_demo/releases/download/v0.1.0/mpiifacegaze_resnet_simple.pth

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://github.com/hysts/pytorch_mpiigaze_demo/releases/download/v0.1.0/mpiifacegaze_resnet_simple.pth ，下载原始weights，存放于 `/weights`文件夹中

<div style="background-color: #FFFFCC; color: #000000; padding: 10px; border-left: 5px solid #FFA500;">
注意：有时开源的weights url可能会变更。如果我们提供的weights url失效，请根据原工程相应的branch以及commit版本寻找正确的下载链接
</div>

2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_MPIIFaceGaze

```shell
# 在此模型根目录
mkdir 0_MPIIFaceGaze
git clone -b master https://github.com/hysts/pytorch_mpiigaze.git 0_MPIIFaceGaze
cd 0_MPIIFaceGaze
git checkout 2a31a6c
```

3）根据源码readme构建环境，进入0_MPIIFaceGaze下，打开wsl环境，运行`bash scripts/download_dlib_model.sh`下载`shape_predictor_68_face_landmarks.dat`，下载好后该文件会放在0_MPIIFaceGaze/data/dlib/目录下

4）将1_scripts中的脚本复制到0_MPIIFaceGaze路径下，在0_MPIIFaceGaze路径下执行保存模型脚本，会将保存好的模型放在2_compile/fmodel路径下。
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/MPIIFaceGaze_8.toml
     ```

     如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）

     其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/MPIIFaceGaze/imodel

# 3. 仿真

通过配置3_deploy/modelzoo/MPIIFaceGaze/cfg/中yaml文件的sim字段为True实现模型仿真。
...
1. **python运行虚拟环境要求与准备**
   ...
   - 在选定的python运行虚拟环境中安装python运行时所需要的依赖包
     ...
     ```
     cd 3_deploy/modelzoo/MPIIFaceGaze
...
2. **执行程序**
   ...
   ```
      python ./MPIIFaceGaze_psin.py ../cfg/MPIIFaceGaze_psin.yaml
   ```

   	在io/output中查看结果。

   ​     此外，还提供一个可视化demo，需先配置好0_MPIIFazeGaze的环境，再在此虚拟环境中按照`1.python运行虚拟环境要求与准备`搭建好环境，并在运行前将0_MPIIFaceGaze/data/dlib路径下的shape_predictor_68_face_landmarks.dat复制到3_deploy\modelzoo\MPIIFaceGaze\pyrt\data\dlib路径下，再执行下面命令：

   ```
   python ./MPIIFaceGaze_psin_demo.py ../cfg/MPIIFaceGaze.yaml
   ```

   运行该demo能提供可视化结果，并将结果保存至io/output中。
...
1. **重新编译模型**
   - 1）由于源码工程未提供精度测试的预训练模型，可运行0_MPIIFaceGaze/train.py训练模型，也可使用已重训好的权重`checkpoint_0015.pth`
   
   - 2）使用`checkpoint_0015.pth`权重重新运行1_save.py，导出模型
...
2. **数据集准备**
   ...
   * 测试数据集下载：[MPIIFaceGaze](http://datasets.d2.mpi-inf.mpg.de/MPIIGaze/MPIIFaceGaze_normalized.zip)
   * 模型测试使用MPIIFazeGaze的`p00`作为测试集，将下载下来的数据集MPIIFaceGaze存放于  `0_MPIIFaceGaze/datasets`中，并解压到当前文件夹

3. **修改运行时的yaml配置文件**

   运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在 `3_deploy/modelzoo/MPIIFaceGaze/cfg/`

   测试时使用：`MPIIFaceGaze_psin_test.yaml`
...
4. **编译、执行运行时程序**
   ...
   ```
   cd 3_deploy/modelzoo/MPIIFaceGaze/pyrt
   python ./MPIIFaceGaze_psin.py ../cfg/MPIIFaceGaze_psin.yaml
   ```

   程序结束自动计算出精度指标平均angle_error，并记录在MPIIFaceGaze_metrics.log中


# 6. 模型性能记录

以下是基于自训权重`checkpoint_0015.pth`的精度测试结果：

| MPIIFaceGaze | input shape      | hard time | qt_strategy | 精度(mean angle error) |
| --------------- | ---------------- | --------- | ------------ | ------------ |
| float | [1, 3, 224,224] | -         | -           | 2.55                   |
| parse | [1, 3, 224,224] | -         | -           | 2.20                   |
| int8 | [1, 3, 224,224] | 1.3355    | null-pc     | 3.22                   |
| int16 | [1, 3, 224,224] | 2.1721    | null-pc     | 3.25                   |
