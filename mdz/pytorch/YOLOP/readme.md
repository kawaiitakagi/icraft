...
- code：https://github.com/hustvl/YOLOP.git
- branch：main
- commit_id：8d8f68d
- weights：https://github.com/hustvl/YOLOP/blob/main/weights/End-to-end.pth

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）权重地址：https://github.com/hustvl/YOLOP/blob/main/weights/End-to-end.pth ，下载好源码后权重会一同下载至源码的 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_YOLOP

```shell
# 在此模型根目录
mkdir 0_YOLOP
git clone -b main https://github.com/hustvl/YOLOP.git 0_YOLOP
cd 0_YOLOP
git checkout 8d8f68d
```

3）按照源码readme搭建好环境后，将1_scripts中的文件复制到0_YOLOP中，执行保存模型脚本
...
- 1_save.py                              >保存模型，保存好的用于Icraft编译器的模型，会存放在 `/2_compile/fmodel`
  ...
  2. 去除detect头conv后面的一些计算，将其放置后处理部分；<br>
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/YOLOP_psin_8.toml
     ```

     如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）

     其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/YOLOP/imodel

# 3. 仿真

通过配置3_deploy/modelzoo/YOLOP/cfg/中yaml文件的sim字段为True实现模型仿真。
...
2. **执行程序**
   ...
   ```
   python ./yolop_psin.py ../cfg/yolop.yaml
   ```
...
* 测试数据集下载：[BDD-100k](https://dl.cv.ethz.ch/bdd100k/data/)

* 模型测试使用BDD100k image数据集的Val部分，**按照以下层级关系**存放测试集图片以及各类任务标签。
  ...
  ```
      ...
      │   ├───train
      │   └───val
      ├───da_seg_annot  # 可行驶区域分割任务标签
      │   ├───train
      │   └───val
      └───ll_seg_annot  # 车道线检测任务标签
          ├───train
          └───val
  ```
...
3. **修改运行时的yaml配置文件**
   ...
   运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在 `3_deploy/modelzoo/YOLOP/cfg/`

   测试时使用：`yolop_test.yaml`

   需要修改：

   * imodel中参数：修改精度测试所用模型文件路径`dir`，根据所使用板子ip设置`ip`。
   * dataset中参数：更改自己的精度测试数据集路径`dir`
...
4. **编译、执行运行时程序**
   ...
   ```
   cd 3_deploy/modelzoo/YOLOP/pyrt
   python ./yolop_psin_test.py ../cfg/yolop_test.yaml
   ```

   运行结束即可生成精度测试结果，存放于`YOLOP_metrics.log`文件中。
   

# 6. 模型性能记录

| YOLOP    | input shape      | hard time | qt_strategy | Traffic Object Detection   | Drivable Area Segmentation | Lane Line Detection        |
| --------------- | ---------------- | --------- | ------------ | --------------- | --------------- | --------------- |
| float    | [1, 3, 640, 640] | -         | -           | recall:89.3<br/>map50:76.6 | mIoU:89.0                  | accuracy:63.8<br/>IoU:24.8 |
| parse    | [1, 3, 640, 640] | -         | -           | recall:89.3<br/>map50:76.6 | mIoU:89.0                  | accuracy:63.8<br/>IoU:24.8 |
| BY-8bit  | [1, 3, 640, 640] | 24.87ms   | kld-pc      | recall:88.9<br/>map50:74.9 | mIoU:88.7                  | accuracy:60.5<br/>IoU:24.9 |
| BY-16bit | [1, 3, 640, 640] | 47.30ms   | kld-pc      | recall:89.2<br/>map50:76.6 | mIoU:89.0                  | accuracy:64.9<br/>IoU:24.9 |

注意：float为重测的源码精度，其中目标检测任务和源码对齐，但分割任务均未能复现源码readme中的精度。