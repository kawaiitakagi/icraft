...
class AutoDriveDataset(Dataset):
    ...
    def __init__(self, cfg, is_train, inputsize=640, transform=None):
        ...
        img_root = Path(cfg["dataset"]["data_root"])
        label_root = Path(cfg["dataset"]["labelroot"])
        mask_root = Path(cfg["dataset"]["da_segroot"])
        lane_root = Path(cfg["dataset"]["ll_segroot"])
        indicator = "val"
    ...
    def __getitem__(self, idx):
        ...
        (img, seg_label, lane_label), ratio, pad = letterbox((img, seg_label, lane_label), resized_shape, auto=False, scaleup=self.is_train)
...
#        _,seg2 = cv2.threshold(seg_label[:,:,2],1,255,cv2.THRESH_BINARY)
        ...
        # img = self.transform(img)#/255、torch.tensor、HWC->CHW
