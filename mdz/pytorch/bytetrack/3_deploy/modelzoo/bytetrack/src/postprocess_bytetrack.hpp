...
void post_detpost_hard(std::vector<int> real_out_channels, const std::vector<Tensor>& output_tensors, PicPre_bytetrack& img, NetInfo& netinfo, std::vector<std::vector<float>>norm,
    ...
    for (size_t i = 0; i < output_tensors.size(); i++) {
        ...
        int output_tensors_bits = netinfo.detpost_bit; //output_tensors[i].dtype()->element_dtype.getStorageType().bits();
    ...
    //-------------VIS OR SAVE-----------------------//
        ...
        if (save) {
            ...
            std::regex reg(R"(\.(\w*)$)");
            std::string result_file = std::regex_replace(save_path, reg, ".txt");
    ...
    #elif __linux__
        string save_path = resRoot + '/' + std::to_string(frame_id) + "_result" + ".jpg";
    ...
    for (int yolo = 0; yolo < output_tensors.size() - 1; yolo = yolo + 3) {
        ...
        for (size_t h = 0; h < _H; h++) {
            ...
            for (size_t w = 0; w < _W; w++) {
                ...
                //std::cout << "one_head_stride:" << one_head_stride << std::endl;
...
YoloPostResult post_detpost_plin(std::vector<int> real_out_channels, const std::vector<Tensor>& output_tensors, YoloPostResult& last_frame_result, NetInfo& netinfo, std::vector<std::vector<float>>norm,
    float conf, float iou_thresh, bool MULTILABEL, bool fpga_nms, int N_CLASS, float ALPHA, float SMOOTH_IOU,
    ...
    //std::cout << "After nms num is : " << nms_res.size() << std::endl;
