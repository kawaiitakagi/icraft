...
1）根据模型来源中的地址：https://github.com/WongKinYiu/yolov9/releases/download/v0.1/yolov9-t-converted.pt ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_yolov9

```shell
# 在此模型根目录
mkdir 0_yolov9
git clone -b main https://github.com/WongKinYiu/yolov9 0_yolov9
cd 0_yolov9
...
- 1_save.py                              >保存模型，保存好的用于Icraft编译器的模型，会存放在 `/2_compile/fmodel`
  ...
  2. 将DDetect类的forward函数修正为仅保留至conv算子，去掉concat及之后的操作<br>
  3. 将模型bbox分支的分组卷积后添加identity作用的conv算子<br>
  </div>
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/yolov9t_8.toml
     ```
  

​	如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）
​	其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/yolov9/imodel

# 3. 仿真

通过配置3_deploy/modelzoo/yolov9/cfg/中yaml文件的sim字段为True实现模型仿真。
```json
imodel:
   ...
   sim: true
...
1. **编译环境准备**
   ...
   **编译c++程序**
   ...
   ```shell
   #在3.1所需的linux编译环境中
   cd 3_deploy/modelzoo/yolov9/build_arm
   cmake ..
   make -j
   ```
...
5. **执行程序**
   ...
   ```
   cd /home/fmsh/ModelZoo/modelzoo/yolov9/build_arm
   chmod 777 *
   ./yolov9 ../cfg/yolov9.yaml
   ```
...
* 模型测试使用COCO2017数据集的Val部分，测试图片存放于  `3_deploy\modelzoo\yolov9\io\input`及`3_deploy\modelzoo\yolov9\io\cocoval_2017.txt`中
* 如需测试其他数据集，需自行将图片存放于`3_deploy\modelzoo\yolov9\io\input`,并准备对应的input.txt



3. **修改 运行时的yaml配置文件**：

   运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在`3_deploy/modelzoo/yolov9/cfg/`

   测试时使用：`yolov9_test.yaml`
...
```shell
cd 3_deploy/modelzoo/yolov9
mkdir build_win
cd build_win
cmake ..
cmake --build . --config Release
./Release/yolov9.exe ../cfg/yolov9_test.yaml
```

注意：如果使用visualstudio打开sln进行调试，需要右键工程-属性-调试-环境中配置`./Release/yolov9.exe ../cfg/yolov9_test.yaml`
...
| yolov9 | input shape     | hard time      | qt_strategy     | 精度             |
| -------------- | --------------- | -------------- | --------------- | ---------------- |
| float          | [1, 3, 640, 640] | -              | -               | ap50:53.1,ap50-95:38.3 |
| 8bit           | [1, 3, 640, 640] | 8.46ms  | kld-pc | ap50:49.8,ap50-95:35.2 |
| 16bit          | [1, 3, 640, 640] | 12.337ms | kld-pc | ap50:52.4,ap50-95:37.2 |
