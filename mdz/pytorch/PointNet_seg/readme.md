...
- code：https://github.com/saaries/PointNet
- branch：master
- commit_id：16af8ee
- weights：https://download.fdwxhb.com/data/04_FMSH-100AI/100AI/04_modelzoo/modelzoo_pub/weights/pytorch/PointNet_seg/seg_model_Airplane_4.pth

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://download.fdwxhb.com/data/04_FMSH-100AI/100AI/04_modelzoo/modelzoo_pub/weights/pytorch/PointNet_seg/seg_model_Airplane_4.pth ，下载原始weights，存放于 `/weights`文件夹中
...
* 您也可自行训练，将训练好的权重存放于`/weight`文件夹中
</div>

2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_PointNet_seg

```shell
# 在此模型根目录
mkdir 0_PointNet_seg
git clone -b master https://github.com/saaries/PointNet 0_PointNet_seg
cd 0_PointNet_seg
git checkout 16af8ee
```
...
- 1_save.py                              >保存模型，保存好的用于Icraft编译器的模型，会存放在 `/2_compile/fmodel`
  ...
  1. 将 max操作替换为maxpool1d操作`<br>`
  2. 将repeat操作替换为expand操作 `<br>`
  3. 将logsoftmax操作替换为softmax操作 `<br>`
  4. 将forward过程中声明的iden常量迁移至ini过程中`<br>`

  </div>
- 2_save_infer.py                    >用修改后保存的模型做前向推理，验证保存的模型与原模型是否一致
- 3_sim_infer.py                    >用编译后的模型做前向仿真，验证编译后的模型与导出的模型是否一致
- 可视化说明 > 运行上述脚本均会将结果可视化,在可视化窗口并切换为英文输入法下：<br>
  - 'p'表示推理结果
  - 't'表示真值结果(ground_truth)
  - 's'表示保存当前图片
  - 'q'表示退出可视化窗口
  - 更多操作请参考`PointNet_seg_utils/showpoints`定义
- render_balls_so.dll问题排查及解决方法
  - 问题描述：运行1_scripts中的脚本时，报`0_PointNet_seg/utils/render_balls_so.dll`找不到
  - 解决方案：请从源机器或开源网站下载相关库(MSVCR100.dll和MSVCP100.dll) 拷贝至目标机器相应文件夹，库分32位和64位（跟自己操作系统有关），32拷贝到C:\Windows\System32，64位拷贝到C:\Windows\SysWOW64，详细解决方案请参考[MSVCR100.dll、MSVCR100d.dll、Msvcp100.dll、Msvcp100D.dll 故障检查及解决方法](https://blog.csdn.net/bitree1/article/details/111193235)
```shell
# render_balls_so.dll 报错信息
FileNotFound: Could not find module 'xxx\0_PointNet_seg\utils\render_balls_so.dll'. Try using the full path with constructor suntax.
```
# 2.使用Icraft编译器编译模型
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/PointNet_seg_8.toml
     ```

     如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）

     其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/PointNet_seg/imodel

# 3. 仿真

通过配置3_deploy/modelzoo/PointNet_seg/cfg/中yaml文件的sim字段为True实现模型仿真。
...
1. **python运行虚拟环境要求与准备**
   ...
   - 在选定的python运行虚拟环境中安装python运行时所需要的依赖包
     ...
     ```
     cd 3_deploy/modelzoo/PointNet_seg
...
2. **执行程序**
   ...
   * 执行程序前请确保在`3_deploy/modelzoo/PointNet_seg/io/input`文件夹下下载并解压完整的数据集
   * input/shapenetcore_partanno_segmentation_benchmark_v0数据集结构为：
```shell
...
```
   * 执行程序
   ```
   python ./PointNet_seg.py ../cfg/pointnet_seg.yaml
   ```
...
* 测试数据集下载：[shapenet](https://shapenet.cs.stanford.edu/ericyi/shapenetcore_partanno_segmentation_benchmark_v0.zip)
* 模型测试使用shapenet数据集的test部分，测试图片存放于  `3_deploy\modelzoo\PointNet_seg\io\input\input\shapenetcore_partanno_segmentation_benchmark_v0`中
* 如需测试其他数据集，需自行将图片存放于 `3_deploy\modelzoo\PointNet_seg\io\input`


3. **修改 运行时的yaml配置文件**：

   运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在 `3_deploy/modelzoo/PointNet_seg/cfg/`

   测试时使用：`pointnet_seg_test.yaml`

   需要修改：

   * imodel中参数：修改精度测试所用模型文件路径`dir`，根据所使用板子ip设置`ip`。
   * dataset中参数：更改自己的精度测试数据集路径。
   * param中参数：如果模型没有重训，则无需更改，若 重训则需要修改对应部分参数; 如class_choice表示分割类别，默认分割`Airplane`类；如果重训分割其他类别，需要修改此参数。
...
4. **编译、执行运行时程序**
 ...
 python运行时（不需要编译）：
   ...
   ```
   cd 3_deploy/modelzoo/PointNet_seg/pyrt
   python ./PointNet_seg.py ../cfg/pointnet_seg.yaml
   ```
   运行结束，mIoU为分割精度。
# 6. 模型性能记录

| PointNet_seg | input shape     | hard time | qt_strategy        | 精度        |
| -------------- | --------------- | --------- | ----------- |----------- |
|float|[1,3,2500]|-|-|mIoU:66.9|
|8bit|[1,3,2500]|58.391ms|[kld-pc]|mIoU:66.5|
|16bit|[1,3,2500]|74.320ms|[kld-pc]|mIoU:66.9|
