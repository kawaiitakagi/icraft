...
- branch：master
- commit_id：65ecf14
- weights：https://github.com/ultralytics/assets/releases/download/v8.2.0/yolov8s-cls.pt

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://github.com/ultralytics/assets/releases/download/v8.2.0/yolov8s-cls.pt ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_yolov8_cls

```shell
# 在此模型根目录
mkdir 0_yolov8_cls
git clone -b master https://github.com/ultralytics/ultralytics.git 0_yolov8_cls
cd 0_yolov8_cls
git checkout 65ecf14
```
...
- 1_save.py                              >保存模型，保存好的用于Icraft编译器的模型，会存放在 `/2_compile/fmodel`
  ...
  1. 可以视情况选择new_Classify_forward来去除模型最后的softmax层<br>
  </div>
  
- 2_save_infer.py                    >用修改后保存的模型做前向推理，验证保存的模型与原模型是否一致

- 3_sim_soft.py                    >基于python的icraft推理仿真，用于初步验证编译后模型效果
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/yolov8s_cls_8.toml
     ```
  
      如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）
  
      其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/yolov8_cls/imodel
  
# 3. 仿真

通过配置3_deploy/modelzoo/yolov8_cls/cfg/中yaml文件的sim字段为True实现模型仿真。
```json
imodel:
   ...
   sim: true
...
**编译c++程序**
   ...
   ```shell
   #在3.1所需的linux编译环境中
   cd 3_deploy/modelzoo/yolov8_cls/build_arm
   cmake ..
   make -j
   ```
...
5. **执行程序**
   ...
   ```
   cd /home/fmsh/ModelZoo/modelzoo/yolov8_cls/build_arm
   chmod 777 *
   ./yolov8_cls ../cfg/yolov8_cls.yaml
   ```
...
* 测试数据集下载：[imagenet2012val](https://image-net.org/download.php)

* 模型测试使用imagenet2012数据集的Val部分，测试图片存放于  `3_deploy\modelzoo\yolov8_cls\io\input`及`3_deploy\modelzoo\yolov8_cls\io\list.txt`中
* 如需测试其他数据集，需自行将图片存放于`3_deploy\modelzoo\yolov8_cls\io\input`,并准备对应的input.txt



3. **修改 运行时的yaml配置文件**：

   运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在`3_deploy/modelzoo/yolov8_cls/cfg/`

   测试时使用：`yolov8_cls.yaml`
...
```shell
cd 3_deploy/modelzoo/yolov8_cls
mkdir build_win
cd build_win
cmake ..
cmake --build . --config Release
./Release/yolov8_cls.exe ../cfg/yolov8_cls.yaml
```

注意：如果使用visualstudio打开sln进行调试，需要右键工程-属性-调试-环境中配置`./Release/yolov8_cls.exe ../cfg/yolov8_cls.yaml`
...
| yolov8_cls | input shape     | hard time      | qt_strategy     | 精度             |
| -------------- | --------------- | -------------- | --------------- | ---------------- |
| float          | [1, 3, 224, 224] | -              | -               | top1:72.71%, top5:91.09% |
| int8           | [1, 3, 224, 224] | 0.93041ms | ems-pc | top1:69.33%, top5:88.66% |
| int16          | [1, 3, 224, 224] | 1.94144ms | ems-pc | top1:71.39%, top5:90.29% |
