...
- code：https://github.com/xuebinqin/U-2-Net.git
- branch：master
- commit_id：53dc9da
...
1）根据模型来源中的地址：https://drive.google.com/file/d/1ao1ovG1Qtx4b7EoskHXmi2E9rp5CHLcZ/view ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_U2Net_det

```shell
# 在此模型根目录
mkdir 0_U2Net_det
git clone -b master https://github.com/xuebinqin/U-2-Net.git 0_U2Net_det
cd 0_U2Net_det
git checkout 53dc9da
```
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/U2Net_det_8.toml
     ```
  
      如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）
  
      其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/U2Net_det/imodel
  
# 3. 仿真

通过配置3_deploy/modelzoo/U2Net_det/cfg/中yaml文件的sim字段为True实现模型仿真。
```json
imodel:
   ...
   sim: true
...
**编译c++程序**
   ...
   ```shell
   #在3.1所需的linux编译环境中
   cd 3_deploy/modelzoo/U2Net_det/build_arm
   cmake ..
   make -j
   ```
...
5. **执行程序**
   ...
   ```
   cd /home/fmsh/ModelZoo/modelzoo_v3.1.1/U2Net_det/build_arm
   chmod 777 *
   ./U2Net_det_psin ../cfg/U2Net_det.yaml
   ```
...
1. **数据集准备**
   ...
   测试数据集下载：[DUTS-TE]([💛:dataset_url])

   - 模型测试使用DUTS-TE数据集，测试图片存放于  `3_deploy\modelzoo\U2Net_det\io\DUTS-TE\DUTS-TE-Image\`文件夹下，对应的list文件存放于`3_deploy\modelzoo\U2Net_det\io\DUTS-TE\list.txt`中
   - 如需测试其他数据集，需自行将图片存放于`3_deploy\modelzoo\U2Net_det\io\input`,并准备对应的input.txt

2. **修改运行时的yaml配置文件**：

   运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在`3_deploy/modelzoo/U2Net_det/cfg/`

   测试时使用：`U2Net_det_test.yaml`中的参数设置
...
3. **编译、执行运行时程序**
   ...
   ```shell
   cd 3_deploy/modelzoo/U2Net_det
   mkdir build_win
   cd build_win
   cmake ..
   cmake --build . --config Release
   ./Release/U2Net_det_psin.exe ../cfg/U2Net_det_test.yaml
   ```

   注意：如果使用visualstudio打开sln进行调试，需要右键工程-属性-调试-命令参数中配置` ../cfg/U2Net_det_test.yaml`
...
4. **使用metrics测试脚本，将保存的测试结果计算出精度指标**
   ...
   本模型使用`./MAE/Saliency_Evaluation.py`，在使用时需要指定测试结果的路径，示例如下：

   ```
   python  Saliency_Evaluation.py  --result_pic  ../io/output/8/
   ```
...
| U2Net_det | input shape     | hard time      | qt_strategy     | 精度             |
| -------------- | --------------- | -------------- | --------------- | ---------------- |
| float          | [1, 3, 320, 320] | -              | -               | MAE:4.40%; Max-F:87.30%; weighted-F:80.40% |
| 8bit           | [1, 3, 320, 320] | 37.7630ms | kld-pt | MAE:4.79%; Max-F:86.70%; weighted-F:79.04% |
| 16bit          | [1, 3, 320, 320] | 72.7427ms | kld-pt | MAE:4.76%; Max-F:86.69%; weighted-F:79.19% |
