...
- branch：main
- commit_id：da3e60d
- weights：https://dl.fbaipublicfiles.com/maskformer/panoptic-coco/maskformer_panoptic_swin_tiny_bs64_554k/model_final_539394.pkl

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://dl.fbaipublicfiles.com/maskformer/panoptic-coco/maskformer_panoptic_swin_tiny_bs64_554k/model_final_539394.pkl，下载原始weights，存放于 `/weights`文件夹中
...
也可以安装maskformer源码中的安装说明进行安装，只需要在clone detectron2 git 源码后切换到指定的commitid进行安装即可。<br>

另外panopticapi的安装可以参考下列步骤
```
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/maskformer_pan_640x640_8.toml
     ```

     如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）

     其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/maskformer_pan/imodel

# 3. 仿真

通过配置3_deploy/modelzoo/maskformer_pan/cfg/中yaml文件的sim字段为True实现模型仿真。
...
2. **执行程序**
   ...
   ```
   python ./maskformer_pan_psin.py ../cfg/maskformer_pan.yaml
   ```
...
运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在 `3_deploy/modelzoo/maskformer_pan/cfg/`

测试时使用：`maskformer_pan_test.yaml`

需要修改：

* imodel中参数：根据所使用板子ip设置url_win，
* 更改自己的dataset精度测试数据集路径。
  * 例如下载好的coco，存放路径是“E:\Dataset\coco”，则修改为“dir”路径为“E:\Dataset\coco”
  * coco文件夹下除了val2017的5000张图片外，还需要panoptic_val2017文件夹内的图片标签和对应在annotations文件夹内的json标注信息
...
```
cd 3_deploy/modelzoo/maskformer_pan/pyrt
python ./maskformer_pan_psin.py ../cfg/maskformer_pan_test.yaml
```

运行完之后，可以在命令窗口看到打印的PQ精度信息 

# 6. 模型性能记录

| maskformer | input shape      | hard time | 精度         |
| --------------- | ---------------- | --------- | ------------ |
|                 | [1, 3, 224, 224] | {'8bit': '490.7640ms', '16bit': '1170.6667ms', 'mixed': None} | {'float': ['PQ:44.988'], 'parse': ['PQ:44.988'], '8bit': [kld-pc,PQ:18.217], '16bit': [kld-pc,PQ:None], 'mixed': None} |
