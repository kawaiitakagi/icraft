...
from detectron2.data.datasets.builtin import register_all_ade20k,_get_builtin_metadata,register_coco_panoptic
from detectron2.data import DatasetCatalog, MetadataCatalog
DatasetCatalog.remove("coco_2017_val_panoptic")
MetadataCatalog.remove("coco_2017_val_panoptic")
DatasetCatalog.remove("coco_2017_val_panoptic_separated")
MetadataCatalog.remove("coco_2017_val_panoptic_separated")
...
def test4icraft(cls, cfg, session,run_sim,netinfo,network,device ,evaluators=None):
    ...
    for idx, dataset_name in enumerate(cfg.DATASETS.TEST):
        ...
        for idx, inputs in enumerate(data_loader):
            ...
            mask_pred_result = sem_seg_postprocess(mask_pred_result, image.image_sizes[0], inputs[0]['height'], inputs[0]['width'])
            ...
            outputs.append({"sem_seg": sem_seg})

            panoptic_r = panoptic_inference(mask_cls_result, mask_pred_result)
            outputs[-1]["panoptic_seg"] = panoptic_r
...
""",
    ...
    parser.add_argument("--config-file", default="configs/coco-panoptic/swin/maskformer_panoptic_swin_tiny_bs64_554k.yaml", metavar="FILE", help="path to config file")
    parser.add_argument(
        "--resume",
        action="store_true",
...
def main(session,run_sim,netinfo,network,device,datasetRoot):
    ...
    register_coco_panoptic(
        ...
        datasetRoot+'\\val2017',
        os.path.join(datasetRoot, "panoptic_val2017"),
        os.path.join(datasetRoot, "annotations\\panoptic_val2017.json"),
       datasetRoot+ '\\annotations\\instances_val2017.json',
    )
