...
- 0_ShuffleNet    >模型原始工程，需要自行下载(本模型无需准备此文件夹)
...
- code：https://github.com/pytorch/vision/blob/main/torchvision/models/shufflenetv2.py
- branch：main
- commit_id：f02caa5
...
1）根据模型来源中的地址：https://download.pytorch.org/models/shufflenetv2_x1-5666bf0f80.pth ，下载原始weights，存放于 `/weights`文件夹中
...
- 2_save_infer.py                    >用保存的模型做前向推理，验证保存的模型与原模型是否一致
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/ShuffleNetv2_8.toml
     ```
  
      如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）
  
      其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/ShuffleNet/imodel
  
# 3. 仿真

通过配置3_deploy/modelzoo/ShuffleNet/cfg/中yaml文件的sim字段为True实现模型仿真。
```json
imodel:
   ...
   sim: true
...
**编译c++程序**
   ...
   ```shell
   #在3.1所需的linux编译环境中
   cd 3_deploy/modelzoo/ShuffleNet/build_arm
   cmake ..
   make -j
   ```
...
5. **执行程序**
   ...
   ```
   cd /home/fmsh/ModelZoo/modelzoo/ShuffleNet/build_arm
   chmod 777 *
   ./ShuffleNet ../cfg/ShuffleNet.yaml
   ```
...
* 测试数据集下载：[ImageNet](https://image-net.org/download.php)

* 模型测试使用ImageNet数据集的Val部分，测试图片存放于  `3_deploy\modelzoo\ShuffleNet\io\input`及`3_deploy\modelzoo\ShuffleNet\io\ImageNet2012_Test5000_imgList.txt`中
* 如需测试其他数据集，需自行将图片存放于`3_deploy\modelzoo\ShuffleNet.0\io\input`,并准备对应的input.txt



3. **修改 运行时的yaml配置文件**：

   运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在`3_deploy/modelzoo/ShuffleNet/cfg/`

   测试时使用：`ShuffleNet_test.yaml`
...
```shell
cd 3_deploy/modelzoo/ShuffleNet
mkdir build_win
cd build_win
cmake ..
cmake --build . --config Release
./Release/ShuffleNet.exe ../cfg/ShuffleNet_test.yaml
```

注意：如果使用visualstudio打开sln进行调试，需要右键工程-属性-调试-环境中配置`./Release/ShuffleNet.exe ../cfg/ShuffleNet_test.yaml`
...
- 本模型使用`./topn/topn_acc.py` 
- result_txt字段设置为片上系统预测结果存放路径



# 6. 模型性能记录

| ShuffleNet | input shape     | hard time      | qt_strategy     | 精度             |
| -------------- | --------------- | -------------- | --------------- | ---------------- |
| float          | [1, 3, 224, 224] | -              | -               | top-1:69.88 |
| 8bit           | [1, 3, 224, 224] | 4.067 ms  | kld-pc | top-1:67.14  |
| 16bit          | [1, 3, 224, 224] | 3.791 ms | kld-pc | top-1:68.00 |
