...
- code：https://github.com/guanyuezhen/A2Net.git
- branch：main
- commit_id：08b3b85
- weights：https://github.com/guanyuezhen/A2Net/blob/main/results_LEVIR_iter_40000_lr_0.0005/best_model.pth

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）下载源代码时，权重文件会直接下载，并存放于./results_LEVIR_iter_40000_lr_0.0005/best_model.pth 中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_A2Net

```shell
# 在此模型根目录
mkdir 0_A2Net
git clone -b main https://github.com/guanyuezhen/A2Net.git 0_A2Net
cd 0_A2Net
git checkout 08b3b85
```
...
- 1_save.py                              >保存模型，保存好的用于Icraft编译器的模型，会存放在 `/2_compile/fmodel`中
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/A2Net_8.toml
     ```
  
      如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）
  
      其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/A2Net/imodel
  
# 3. 仿真

通过配置3_deploy/modelzoo/A2Net/cfg/中yaml文件的sim字段为True实现模型仿真。
```json
imodel:
   ...
   sim: true
...
**编译c++程序**
   ...
   ```shell
   #在3.1所需的linux编译环境中
   cd 3_deploy/modelzoo/A2Net/build_arm
   cmake ..
   make -j
   ```
...
5. **执行程序**
   ...
   ```
   cd /home/fmsh/ModelZoo/modelzoo_v3.0.1/A2Net/build_arm
   chmod 777 *
   ./A2Net_psin ../cfg/A2Net.yaml
   ```
...
| A2Net | input shape     | hard time      | qt_strategy     | 精度             |
| -------------- | --------------- | -------------- | --------------- | ---------------- |
| float          | [1, 3, 256, 256] | -              | -               | None |
| 8bit           | [1, 3, 256, 256] | 6.0586ms | None | None  |
| 16bit          | [1, 3, 256, 256] | 10.3644ms | None | None |
