...
#include "post_process_yolov9_pan.hpp"
...
int main(int argc, char* argv[])
{
	...
	for (auto name : namevector) {
		...
		img.Resize({ netinfo.i_cubic[0].h, netinfo.i_cubic[0].w }, PicPre::LONG_SIDE).rPad(1);
		...
		//std::cout << outputs[9].dtype()->shape << std::endl;
		//std::cout << outputs[10].dtype()->shape << std::endl;
		if (!run_sim) { 
			device.reset(1);
