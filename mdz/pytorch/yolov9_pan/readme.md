...
- code：https://github.com/WongKinYiu/yolov9.git
- branch：main
- commit_id：5b1ea9a
- weights：https://github.com/WongKinYiu/yolov9/releases/download/v0.1/gelan-c-pan.pt

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://github.com/WongKinYiu/yolov9/releases/download/v0.1/gelan-c-pan.pt ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_yolov9_pan

```shell
# 在此模型根目录
mkdir 0_yolov9_pan
git clone -b main https://github.com/WongKinYiu/yolov9.git 0_yolov9_pan
cd 0_yolov9_pan
git checkout 5b1ea9a
```
...
- 1_save.py                              >保存模型，保存好的用于Icraft编译器的模型，会存放在 `/2_compile/fmodel`
  ...
  1. 将BaseModel类的forward_once修正为能导出tracedmodel的new_predict_once `<br>`
  2. 将Panoptic类的forward函数修正为仅保留至conv算子，去掉concat及之后的操作 `<br>`

  </div>
- 2_save_infer.py                    >用修改后保存的模型做前向推理，验证保存的模型与原模型是否一致
- 3_sim_infer.py                     >基于python的icraft推理仿真，用于初步验证编译后模型效果
- 4_generate_gt.py                >基于COCO数据集生成测试用的全景ground truth
- 5_test.py                             >可用于精度测试，定义了如何计算全景分割的PQ结果
...
​	其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/yolov9_pan/imodel

# 3. 仿真

通过配置3_deploy/modelzoo/yolov9_pan/cfg/中yaml文件的sim字段为True实现模型仿真。
...
3. **编译c++程序**
   ...
   ```shell
   #在3.1所需的linux编译环境中
   cd 3_deploy/modelzoo/yolov9_pan/build_arm
   cmake ..
   make -j
   ```
...
5. **执行程序**
   ...
   ```
   cd /home/fmsh/ModelZoo/modelzoo/yolov9_pan/build_arm
   chmod 777 *
   ./yolov9_pan_psin ../cfg/yolov9_pan_psin.yaml
   ```
...
3. **修改 运行时的yaml配置文件**：
   ...
   运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在 `3_deploy/modelzoo/yolov9_pan/cfg/`

   测试时使用：`yolov9_pan_psin_test.yaml`
...
```shell
cd 3_deploy/modelzoo/yolov9_pan
mkdir build_win
cd build_win
cmake ..
cmake --build . --config Release
./Release/yolov9_pan_psin.exe ../cfg/yolov9_pan_psin_test.yaml
```

注意：如果使用visualstudio打开sln进行调试，需要右键工程-属性-调试-环境中配置 `./Release/yolov9_pan_psin.exe ../cfg/yolov9_pan_psin_test.yaml`
...
| yolov9_pan | input shape      | hard time | qt_strategy | 精度    |
| ---------- | ---------------- | --------- | ----------- | ------- |
| float      | [1, 3, 640, 640] | -         | -           | PQ:35.3 |
| int8       | [1, 3, 640, 640] | 41.9832ms | null-pc     | PQ:32.5 |
| int16      | [1, 3, 640, 640] | 82.9947ms | null-pc     | PQ:34.1 |
