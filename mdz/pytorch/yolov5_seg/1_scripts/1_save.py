...
INP_H = 640
INP_W = 640
WEIGHTS_PATH = '../weights/yolov5s-seg.pt'
IMAGE_PATH = '../2_compile/qtset/coco'
# TRACED_MODEL_PATH = "../2_compile/fmodel/yolov5s_seg_640x640.pt"
TRACED_MODEL_PATH = "../2_compile/fmodel/yolov5s_seg_"+str(INP_H)+"x"+str(INP_W)+".pt"
...
def export_torchscript(model, im, file, optimize, prefix=colorstr('TorchScript:')):
    ...
    im = torch.ones(1, 3, INP_H, INP_W, dtype = torch.float32)
    print(im.shape)
...
def run(
    ...
    LOGGER.info(f"\n{colorstr('PyTorch:')} starting from {file} with output shape {y[0].shape} ({file_size(file):.1f} MB)")
    ...
    if any(f):
        ...
        LOGGER.info(f'\nExport complete ({time.time() - t:.1f}s)'
                    f"\nResults saved to {colorstr('bold', TRACED_MODEL_PATH)}"
...
def parse_opt(known=False):
    ...
    parser.add_argument('--data', type=str, default=ROOT / '../0_YoloV5_seg/data/coco128.yaml', help='dataset.yaml path')
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / '../weights/yolov5s-seg.pt', help='model.pt path(s)')
