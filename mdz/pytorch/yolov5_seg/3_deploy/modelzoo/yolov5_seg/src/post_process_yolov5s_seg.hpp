 ...
 */
std::vector<std::tuple<int, float, cv::Rect2f, std::vector<float>>> nms_hard_mask(std::vector<cv::Rect2f>& box_list, std::vector<float>& score_list, std::vector<int>& id_list, std::vector<std::vector<float>>& mask_info, const float& iou, icraft::xrt::Device& device, int max_nms = 3000) {
    ...
    device.defaultRegRegion().write(reg_base + 0x030, 0, true); //mode: 0同类之间筛选、1所有类之间筛选 
    ...
    for (auto idx : nms_indices) {
        num_res.push_back({ id_list[idx],score_list[idx],box_list[idx],mask_info[idx] });
    }
    return num_res;
}

std::vector<std::vector<float>> coordTrans_point(std::vector<std::tuple<int, float, cv::Rect2f, std::vector<float> >>& nms_res, cv::Mat proto_mat_float, PicPre& img, bool check_border = true) {
    ...
    for (auto&& res : nms_res) {
        ...
        if (w > 0 && h > 0) {
            ...
            auto dst_size = (img.src_img.cols >= img.src_img.rows) ? img.src_img.cols : img.src_img.rows;
            ...
            masks_2 = masks_1(cv::Range(0, img.src_img.rows), cv::Range(0, img.src_img.cols));

            cv::Mat mask_tmp = cv::Mat::zeros(cv::Size(img.src_img.cols, img.src_img.rows), CV_8UC1);
            ...
            cv::Mat rgb_mask_test = cv::Mat::zeros(cv::Size(img.src_img.cols, img.src_img.rows), img.src_img.type());
            add(rgb_mask_test, color_, rgb_mask_test, mask_tmp);
            mask = mask + rgb_mask_test;

            cv::Mat out;
            cv::cvtColor(mask, out, cv::COLOR_BGR2GRAY);

            //cv::imshow("findContours", out);
            //cv::waitKey();

            std::vector <std::vector<cv::Point>> points;
            cv::findContours(out, points, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE, cv::Point());
            int id = 0;
            int points_number = 0;
...
void visualize_point(std::vector<std::vector<float>>& output_res, std::vector<std::tuple<int, float, cv::Rect2f, std::vector<float> >>& nms_res, cv::Mat proto_mat_float, const cv::Mat& img, const std::string resRoot, const std::string name, const std::vector<std::string>& names, int& protoh, int& protow) {
    std::default_random_engine e;
    std::uniform_int_distribution<unsigned> u(10, 200);
    int index = 0;
    ...
    for (auto res : output_res) {
        ...
        float ratio_h = (float)img.rows / protoh;
        float ratio_w = (float)img.cols / protow;
        float ratio = (std::max)(ratio_w, ratio_h);

        cv::Mat  masks_1 = out_mask_sigmoid.reshape(1, protoh);
...
std::tuple<std::vector<std::vector<float>>, Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic>> coordTrans_mask(const std::vector<Tensor>& icraft_results, std::vector<std::tuple<int, float, cv::Rect2f, std::vector<float>>>& nms_res, PicPre& img,
    ...
    int top_pad = img.getPad().second;
    float ratio = img.getRatio().first;
    auto proto = icraft_results[3].to(HostDevice::MemRegion());
...
void visualize_mask(std::vector<std::vector<float>>& output_res, Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic>& mask_res, const cv::Mat& img, const std::string resRoot, const std::string name, const std::vector<std::string>& names, int &protoh, int &protow) {
    ...
    for (auto res : output_res) {
        ...
        std::string s = std::to_string(class_id) + "_" + names[class_id] + " " + ss.str();
        auto s_size = cv::getTextSize(s, cv::FONT_HERSHEY_DUPLEX, 0.5, 1, 0);
        cv::rectangle(img, cv::Point(x1 - 1, y1 - s_size.height - 7), cv::Point2f(x1 + s_size.width, y1 - 2), color_, -1);
        cv::putText(img, s, cv::Point2f(x1, y1 - 2), cv::FONT_HERSHEY_DUPLEX, 0.5, cv::Scalar(255, 255, 255), 0.2);
        //获取每个物体的mask
...
void get_cls_bbox_maskInfo(std::vector<int>& id_list, std::vector<float>& socre_list, std::vector<cv::Rect2f>& box_list, std::vector<std::vector<float>>& mask_info, T* tensor_data, int base_addr,
    ...
    std::vector<float> anchor, int N_CLASS, float THR_F, bool MULTILABEL,int mask_channel) {
    if (!MULTILABEL) {
        ...
        if (realscore > THR_F) {
            ...
            int seg_ptr_start = base_addr + 5 + N_CLASS;
            std::vector<float> one_obj_mask_info;
            for (size_t j = 0; j < mask_channel; j++)
            {
                one_obj_mask_info.emplace_back(tensor_data[seg_ptr_start + j] * SCALE);
            }
...
void post_detpost_hard(const std::vector<Tensor>& output_tensors, PicPre& img, NetInfo& netinfo,
    ...
    int& mask_channel, int& protoh, int& protow) {
    ...
    if (fpga_nms && !run_sim) {
        nms_res = nms_hard_mask(box_list, socre_list, id_list, mask_info,  iou_thresh, device);
    }
    else {
    ...
    std::vector<std::vector<float>> output_res = coordTrans_point(nms_res, proto_mat_float, img);
    //std::tuple<std::vector<std::vector<float>>, Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic>> output_res =
    //    coordTrans_mask(output_tensors, nms_res, img, protoh, protow, mask_channel, netinfo.detpost_bit);
#ifdef _WIN32
    if (show) {
        visualize_point(output_res, nms_res, proto_mat_float, img.ori_img, resRoot, name, LABELS, protoh, protow);
...
void post_detpost_soft(const std::vector<Tensor>& output_tensors, PicPre& img, std::vector<std::string>& LABELS,
    ...
    std::vector<std::vector<float>> mask_info;
    std::vector<float> stride = get_stride(netinfo);
    for (int yolo = 0; yolo < output_tensors.size() - 1; yolo++) {
        ...
        for (size_t h = 0; h < _H; h++) {
            ...
            for (size_t w = 0; w < _W; w++) {
                ...
                for (size_t anchor_index = 0; anchor_index < ANCHORS.size(); anchor_index++) {
                    ...
                    auto boxPtr = tensor_data + h * _W * (N_CLASS + 5 + mask_channel) * ANCHORS.size()
                        + w * (N_CLASS + 5 + mask_channel) * ANCHORS.size() + anchor_index * (N_CLASS + 5 + mask_channel);
                    if (!MULTILABEL) {
                        ...
                        auto realscore = _score_ * _prob_;
                        if (realscore > conf) {
                            auto seg_ptr_start = boxPtr + 5 + N_CLASS;
                            std::vector<float> one_obj_mask_info;
                            for (size_t j = 0; j < mask_channel; j++)
                            {
                                one_obj_mask_info.emplace_back(*(seg_ptr_start + j));
                            }
                        ...
                        for (size_t cls_idx = 0; cls_idx < N_CLASS; cls_idx++) {
                            ...
                            auto _score_ = sigmoid(*(boxPtr + 4));
                            auto _prob_ = sigmoid(*(boxPtr + 5 + cls_idx));
                            auto realscore = _score_ * _prob_;
                            if (realscore > conf) {
...
using YoloPostResult = std::tuple<std::vector<int>, std::vector<float>, std::vector<cv::Rect2f>, std::vector<std::vector<float>>, cv::Mat>; // box_list, id_list, score_list ,mask_info

YoloPostResult post_detpost_plin_woc(const std::vector<Tensor>& output_tensors, YoloPostResult last_frame_result, NetInfo& netinfo, std::vector<std::vector<std::vector<float>>>& ANCHORS,
    float conf, float iou_thresh, bool MULTILABEL, bool fpga_nms, int N_CLASS,
    icraft::xrt::Device device, int& mask_channel, int& protoh, int& protow, float& mask_normratio) {
    ...
    std::vector<std::vector<float>> mask_info_ret; //box_list_ret.reserve(nms_indices.size());
    for (auto&& idx_score_bbox : nms_res) {
        // store 
        ...
        mask_info_ret.emplace_back(std::get<3>(idx_score_bbox));
    }
    auto proto_ = output_tensors[0].to(HostDevice::MemRegion());
    auto dims = proto_.dtype()->shape;
    //std::cout << proto_.dtype()->shape << std::endl;
    auto proto = (int8_t*)proto_.data().cptr();
    auto h = dims[2];
    auto w = dims[3];
    auto c = dims[4];

    cv::Mat proto_mat_int8_t = cv::Mat(h * w, c, CV_8S, proto);
    //auto proto_f = proto_mat_float.;
    cv::Mat proto_mat_float;
    proto_mat_int8_t.convertTo(proto_mat_float, CV_32F, mask_normratio);
    //cv::Mat obj_mask_mat = cv::Mat(32, 1, CV_32F, mask_info_ret[0].data());
    //cv::Mat out_mask = proto_mat_float * obj_mask_mat * (-1.0);
    return YoloPostResult{ id_list_ret, score_list_ret, box_list_ret, mask_info_ret ,proto_mat_float };

    //std::vector<std::vector<float>> output_res = coordTrans_point_4plin(nms_res, proto_mat_float,mat, ratio_bias);
    //visualize_point4plin(output_res, nms_res, proto_mat_float, img.ori_img, resRoot, name, LABELS, protoh, protow);

    //Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic> output_res_mask =
    //    Trans_mask_plin_woc(output_tensors[0], nms_res, protoh, protow, mask_channel, mask_normratio, netinfo.detpost_bit);
