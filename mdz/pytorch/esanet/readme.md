...
- branch：main
- commit_id：49d2201
- weights：https://drive.google.com/uc?id=1tviMAEOr-6lJphpluGvdhBDA_FetIR14

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://drive.google.com/uc?id=1tviMAEOr-6lJphpluGvdhBDA_FetIR14 ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_esanet

```shell
# 在此模型根目录
mkdir 0_esanet
git clone -b main https://github.com/TUI-NICR/ESANet 0_esanet
cd 0_esanet
git checkout 49d2201
```
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/esanet.toml
     ```

     如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）

     其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/esanet/imodel

# 3. 仿真

通过配置3_deploy/modelzoo/esanet/cfg/中yaml文件的sim字段为True实现模型仿真。
...
2. **执行程序**
   ...
   ```
   python ./esanet_psin.py ../cfg/esanet_test.yaml
   ```
...
* 测试数据集下载：[sunrgbd](https://rgbd.cs.princeton.edu/)
...
运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在 `3_deploy/modelzoo/esanet/cfg/`

测试时使用：`esanet_test_test.yaml`

需要修改：

* path中参数：根据所使用板子ip设置url_win，更改自己的dataset精度测试数据集路径。
* params中参数：如果模型没有重训，则无需更改，若 重训则需要修改对应部分参数。
...
```
cd 3_deploy/modelzoo/esanet/pyrt
python ./esanet_psin.py ../cfg/esanet_test.yaml
```
...
```shell
cd 3_deploy/modelzoo/esanet
mkdir build_win
cd build_win
cmake ..
cmake --build . --config Release
./Release/esanet_psin.exe ../cfg/esanet_test.yaml
```

注意：如果使用visualstudio打开sln进行调试，需要右键工程-属性-调试-环境中配置 `./Release/esanet_psin.exe ../cfg/esanet_test.yaml`
...
| esanet          | input shape      | hard time       | qt_strategy      | 精度              |
| --------------- | ---------------- | --------------- | ---------------- | ----------------- |
| float           | [1, 3, 480, 640] | -               |          -       | miou, 0.535 |
| int8            | [1, 3, 480, 640] | 52.796ms        |  kld-pc          | miou: 0.514  |
| int16           | [1, 3, 480, 640] | 101.037ms       |  kld-pc          | miou: 0.538 |
