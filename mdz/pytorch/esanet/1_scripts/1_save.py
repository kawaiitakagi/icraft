...
cd 1_script
run: python .\1_save.py --dataset sunrgbd
"""
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt_path', type=str,default='../weights/sunrgbd/r34_NBt1D.pth',
                        required=False,
    ...
    print('-----trace model------')
    torch.onnx.export(model,(image, depth),'../2_compile/fmodel/esanet_480x640_sunrgbd.onnx',opset_version=17)
    trace_model = torch.jit.trace(model,(image,depth))
    torch.jit.save(trace_model,'../2_compile/fmodel/esanet_480x640_sunrgbd.pt')
    ...
    plt.suptitle(f"Image depth" f"Model: {args.ckpt_path}")
    plt.show()
