...
- branch：master
- commit_id：8bcd8e1
- weights：https://download.fdwxhb.com/data/04_FMSH-100AI/100AI/04_modelzoo/modelzoo_pub/weights/pytorch/moderntcn/

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址下载，或者使用Moderntcn工程进行训练，下载或者训练完成后，将weights存放于 `/checkpoints`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_moderntcn，同时按照训练代码readme文件中dataset项下载相关数据集，解压至工程dataset文件夹下
...
```
将Moderntcn内容拷贝至0_moderntcn文件夹下

3）进入1_scripts执行保存模型脚本,此时csv数据会被处理成ftmp数据，保存在qset以及3_deploy下的io文件夹中，用于后续量化、测试。

```shell
# 在此模型根目录,以预测模型为例
cd 1_scripts
python 1_save.py --task_name long_term_forecast --is_training 0 --model_id ETTh1 --model ModernTCN --root_path ../0_moderntcn/ModernTCN-Long-term-forecasting/all_datasets/ETT-small --data_path ETTh1.csv --data ETTh1 --features M --seq_len 336 --pred_len 96 --ffn_ratio 1 --patch_size 8 --patch_stride 4 --num_blocks 1 --large_size 51 --small_size 5 --dims 64 64 64 64 --head_dropout 0.0 --enc_in 7 --dropout 0.3 --itr 1 --train_epochs 100 --batch_size 1 --patience 20 --learning_rate 0.0001 --des Exp --lradj type3 --use_multi_scale False --small_kernel_merged False

# 若进行分类任务
cd 1_scripts
python 1_save.py --task_name classification --is_training 0 --root_path ../0_moderntcn/ModernTCN-classification/all_datasets/Heartbeat/ --data UEA --model_id Heartbeat --model ModernTCN --ffn_ratio 1 --patch_size 8 --patch_stride 4 --num_blocks 1 --large_size 31 --small_size 5 --dims 256 --head_dropout 0.0 --dropout 0.3 --class_dropout 0.0 --itr 1 --learning_rate 0.001 --batch_size 1 --patience 10 --des Exp --use_multi_scale False
...
- 1_save.py                              >保存模型，保存好的用于Icraft编译器的模型，会存放在 `/2_compile/fmodel`
  ...
  1. 将reorg操作用卷积等效<br>
  2. 将IKeypoint类中ImplicitA，ImplicitM的乘和加常数的操作融合到self.m的卷积参数中<br>
  3. 将IKeypoint forward中concat算子之后的操作去掉，并添加identity作用的conv算子<br>
  </div>
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/moderntcn_cls_8.toml
     ```
  
      若进行预测任务，请使用后缀为`ltf`相关toml，分类任务则使用后缀为`cls`相关toml，如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）
  
      其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo_v3.6.0/moderntcn/imodel
  
# 3. 仿真

通过配置3_deploy/modelzoo/moderntcn/cfg/中yaml文件的sim字段为True实现模型仿真。
```json
imodel:
   ...
   sim: true
...
**编译c++程序**
   ...
   ```shell
   #在3.1所需的linux编译环境中
   cd 3_deploy/modelzoo/moderntcn/build_arm
   cmake ..
   make -j
   ```
...
5. **执行程序**
   ...
   ```
   cd /home/fmsh/ModelZoo/modelzoo/moderntcn/build_arm
   chmod 777 *
   ./moderntcn ../cfg/moderntcn_ltf.yaml or ./moderntcn ../cfg/moderntcn_cls.yaml
   ```
...
* 测试数据集下载：[times_series](https://pan.baidu.com/share/init?surl=r3KhGd0Q9PJIUZdfEYoymg&pwd=i9iy)

* 由于c++加载ts、csv较为麻烦，模型测试可使用已经处理好数据集，存放于`3_deploy\modelzoo\moderntcn\io\ltf\input`,分类任务则对应`3_deploy\modelzoo\moderntcn\io\cls\input`



3. **修改 运行时的yaml配置文件**：

   运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在`3_deploy/modelzoo_v3.6.0/moderntcn/cfg/`

   测试时使用：`./moderntcn ../cfg/moderntcn_ltf.yaml or ./moderntcn ../cfg/moderntcn_cls.yaml`
...
```shell
cd 3_deploy/modelzoo/moderntcn
mkdir build_win
cd build_win
cmake ..
cmake --build . --config Release
./Release/moderntcn.exe ../cfg/moderntcn_ltf.yaml
```

注意：如果使用visualstudio打开sln进行调试，需要右键工程-属性-调试-环境中配置`./Release/moderntcn.exe ../cfg/moderntcn_ltf.yaml`
...
- 本模型会在c++工程运行结束后将所有可视化结果及相关指标保存在io/output中



# 6. 模型性能记录

| moderntcn_ltf | input shape     | hard time      | qt_strategy     | mae             |mse       |
| -------------- | --------------- | -------------- | --------------- | ---------------- | ---------------- |
| float          | [1, 336, 7] | -              | -               | 0.394449 | 0.368657 |
| int8           | [1, 336, 7] | 0.72ms | kld-pt | 0.395039 | 0.366876 |
| int16          | [1, 336, 7] | 1.23ms | kld-pt | 0.385568 | 0.355908 |

| moderntcn_cls | input shape     | hard time      | qt_strategy     | acc             |f1       |
| -------------- | --------------- | -------------- | --------------- | ---------------- | ---------------- |
| float          | [1, 405, 61] | -              | -               | 0.765853 | 0.441860 |
| int8           | [1, 405, 61] | 116.05ms | kld-pt | 0.735294 | 0.341463 |
| int16           | [1, 405, 61] | None  | kld-pt | - | - |
