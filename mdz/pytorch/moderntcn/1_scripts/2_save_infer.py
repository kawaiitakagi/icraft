...
def infer_ltf(self, setting, test=0):
        ...
        # self.model.load_state_dict(torch.load(os.path.join('../0_moderntcn/ModernTCN-Long-term-forecasting/checkpoints/' + setting, 'checkpoint.pth')))
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                if self.args.use_amp:
                    ...
                    if 'Linear' in self.args.model or 'TST' in self.args.model:
                        ...
                        x,te,mean,stdev = pre_process(batch_x,batch_x_mark)
                        x = self.model(x.cpu(),mean.cpu(),stdev.cpu())
                        outputs = x
                    else:
                        if self.args.output_attention:
                            outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)[0]
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--model_pt', type=str, required=True, default='model pt', help='path of the pt model')
    parser.add_argument('--model_pt', type=dict, default={'ltf':'../2_compile/fmodel/moderntcn_ltf.pt','cls':'../2_compile/fmodel/moderntcn_cls.pt'}, help='torchscript model path')
