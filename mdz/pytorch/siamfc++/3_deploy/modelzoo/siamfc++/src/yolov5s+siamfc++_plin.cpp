...
// 网络的json&raw文件路径
auto net_0_json_file = "../imodel/plin_yolov5s/yolov5s_plin_BY.json";
auto net_0_raw_file = "../imodel/plin_yolov5s/yolov5s_plin_BY.raw";
...
const std::string MODEL_NAME = "yolov5s+siamfc++";
...
int main(int argc, char* argv[]) {
    ...
    int N_CLASS = 1;  //coco类别号，只检测指定类别；例如N_CLASS = 1，表示只检测person
    ...
    // 加载网络
    auto network_0 = loadNetwork(net_0_json_file, net_0_raw_file);
    ...
    auto image_make = network_0.view(1, 2);// imk
    auto net0_icore = network_0.view(2); // view掉imk
    ...
    NetInfo net0_netinfo = NetInfo(network_0);
    ...
    auto net0_sessions = std::vector<Session>(thread_num);
    ...
    for (int i = 0; i < thread_num; i++) {
        ...
        net0_sessions[i] = Session::Create<BuyiBackend, HostBackend>(net0_icore, { device, HostDevice::Default() });
        ...
        auto buyi_backend_0 = net0_sessions[i]->backends[0].cast<BuyiBackend>();
        ...
        buyi_backend_0.userSetSegment(net2_buf_group[i], Segment::INPUT);
        ...
        net0_sessions[i].enableTimeProfile(true);

        buyi_backend_0.speedMode();
    ...
    //Operation WarpAffine_net0 = getOp_WarpAffine(network_0);
    Operation WarpAffine_net0 = net0_netinfo.WarpAffine_;
    ...
    auto icore_thread = std::thread(
        [&]()
        {
            ...
            while (true) {
                ...
                //-------------------------------------//
                if (!find_person) {
                    auto net0_start = std::chrono::system_clock::now();
                    
                    //net0前向
                    auto net0_tensors = net0_sessions[input_msg.buffer_index].forward(input_msg.image_tensor);
                    
                    auto net0_end = std::chrono::system_clock::now();
                    ...
                    // 目标检测后处理
                    auto hand_post_results = yolov5_post_detpost_plin(net0_tensors, net0_netinfo, conf, iou_thresh, MULTILABEL, N_CLASS, device);
                    find_person = std::get<0>(hand_post_results);
                    std::cout << "find_person: " << find_person << std::endl;
                    cv::Rect bbox = std::get<1>(hand_post_results);
                    std::cout << bbox.tl().x << " " << bbox.tl().y << " " << bbox.width << " " << bbox.height << std::endl;
                    float x0 = bbox.tl().x * RATIO_W + BIAS_W;
                    float y0 = bbox.tl().y * RATIO_H + BIAS_H;
                    float w0 = bbox.width * RATIO_W;
                    float h0 = bbox.height * RATIO_H;
                    init_rect = { x0,y0,w0,h0 };
                    std::cout << "init_rect:" << init_rect[0] << ", " << init_rect[1] << ", " << init_rect[2] << ", " << init_rect[3] << std::endl;
                    
                    auto net0_end_1 = std::chrono::system_clock::now();
                    auto net0_duration_1 = std::chrono::duration_cast<std::chrono::microseconds>(net0_end_1 - net0_end);
                    auto net1_duration_2 = std::chrono::duration_cast<std::chrono::microseconds>(net0_end_1 - net0_end_1);
                    if (find_person) {
                        //net1前处理
                        target_pos = { (init_rect[0] + (init_rect[2] - 1) / 2),(init_rect[1] + (init_rect[3] - 1) / 2) };
                        target_sz = { init_rect[2],init_rect[3] };
                        siamfc_preprocess(target_pos, target_sz, M_inversed, scale, context_amount, z_size, z_size);
                        auto net1_end_2 = std::chrono::system_clock::now();
                        net1_duration_2 = std::chrono::duration_cast<std::chrono::microseconds>(net1_end_2 - net0_end_1);
                    }
                    else {
                    ...
                    #ifdef DEBUG_PRINT
                        spdlog::info("[Icore:net0] net0_forward={:.2f}ms, net0_post={:.2f}ms, net1_pre={:.2f}ms, buffer={}",
                            float(net0_duration.count()) / 1000,
                            float(net0_duration_1.count()) / 1000,
                            float(net1_duration_2.count()) / 1000,
                ...
                if (find_person) {
                    if (initial) {
                        ...
                        auto initop_duration = std::chrono::duration_cast<std::chrono::microseconds>(initop_end - net1_start);
                        ...
                        #ifdef DEBUG_PRINT
                            spdlog::info("[Icore:net1] net1_initop={:.2f}ms, net1_warpaffine={:.2f}ms, net1_forward={:.2f}ms, net1_total={:.2f}ms, net2_pre={:.2f}ms, buffer={}",
                        ...
                        if (save) {
                            ...
                            //cv::cvtColor(camera_frame, cur_frame, cv::COLOR_YUV2RGB_UYVY);
                            // cv::imwrite("thread_result.jpg", cur_frame);
