...
- branch：master
- commit_id：e4df1ec
- weights：https://github.com/ultralytics/yolov5/releases/download/v6.2/yolov5s6.pt

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://github.com/ultralytics/yolov5/releases/download/v6.2/yolov5s6.pt ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_yolov5s6

```shell
# 在此模型根目录
mkdir 0_yolov5s6
git clone -b master https://github.com/ultralytics/yolov5.git 0_yolov5s6
cd 0_yolov5s6
...
- 1_save.py                              >保存模型，保存好的用于Icraft编译器的模型，会存放在 `/2_compile/fmodel`
  ...
  <div style="background-color: #FFFFCC; color: #000000; padding: 10px; border-left: 5px solid #FFA500;">
  保存模型时的修改点：  <br>  1.将Detect forward中conv算子之后的后处理操作去掉：定义new_forward对forward进行替换
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/yolov5s6_1280x1280_deploy_8.toml
     ```
  
      如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）
  
      其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/yolov5s6/imodel
  
# 3. 仿真

通过配置3_deploy/modelzoo/yolov5s6/cfg/中yaml文件的sim字段为True实现模型仿真。
```json
imodel:
   ...
   sim: true
...
**编译c++程序**
   ...
   ```shell
   #在3.1所需的linux编译环境中
   cd 3_deploy/modelzoo/yolov5s6/build_arm
   cmake ..
   make -j
   ```
...
5. **执行程序**
   ...
   ```
   cd /home/fmsh/ModelZoo/modelzoo/yolov5s6/build_arm
   chmod 777 *
   ./yolov5s6 ../cfg/yolov5s6.yaml
   ```
...
3. **修改 运行时的yaml配置文件**：
   ...
   运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在`3_deploy/modelzoo/yolov5s6/cfg/`

   测试时使用：`yolov5s6_test.yaml`
...
```shell
cd 3_deploy/modelzoo/yolov5s6
mkdir build_win
cd build_win
cmake ..
cmake --build . --config Release
./Release/yolov5s6.exe ../cfg/yolov5s6_test.yaml
```

注意：如果使用visualstudio打开sln进行调试，需要右键工程-属性-调试-环境中配置`./Release/yolov5s6.exe ../cfg/yolov5s6_test.yaml`
...
| yolov5s6 | input shape     | hard time      | qt_strategy     | 精度             |
| -------------- | --------------- | -------------- | --------------- | ---------------- |
| float          | [1, 3, 1280, 1280] | -              | -               | ap50:0.637; ap50-95: 0.448|
| int8           | [1, 3, 1280, 1280] | 27.67ms | kld-pc | ap50:0.627; ap50-95:0.406 |
| int16          | [1, 3, 1280, 1280] | 59.36ms | kld-pc | ap50:0.632; ap50-95:0.428 |
