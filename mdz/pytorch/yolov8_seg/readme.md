...
- code：https://github.com/ultralytics/ultralytics.git
- branch：main
- commit_id：437b430
- weights：https://github.com/ultralytics/assets/releases/download/v0.0.0/yolov8s-seg.pt

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://github.com/ultralytics/assets/releases/download/v0.0.0/yolov8s-seg.pt ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_yolov8_seg

```shell
# 在此模型根目录
mkdir 0_yolov8_seg
git clone -b main https://github.com/ultralytics/ultralytics.git 0_yolov8_seg
cd 0_yolov8_seg
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/yolov8s_seg_8.toml # psin 
      # 若要部署plin工程则编译对应的plin模型
   icraft compile config/yolov8s_seg_8_plin.toml # plin 
     ```
  
      如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）
     
      其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/yolov8_seg/imodel
  
# 3. 仿真

通过配置3_deploy/modelzoo/yolov8_seg/cfg/中yaml文件的sim字段为True实现模型仿真。
```json
imodel:
   ...
   sim: true
...
**编译c++程序**
   ...
   # 例如：修改CMakeLists使其编译pin工程
   # set(TARGET_NAME yolov8s_seg_plin) 
   # add_executable(${TARGET_NAME} src/${TARGET_NAME}.cpp)
   cd 3_deploy/modelzoo/yolov8_seg/build_arm
   cmake ..
   make -j
...
5. **执行程序**
   ...
   ```shell
   cd /home/fmsh/ModelZoo/modelzoo/yolov8_seg/build_arm
   chmod 777 *
   ./yolov8s_seg_psin ../cfg/yolov8s_seg_psin.yaml # psin
   ./yolov8s_seg_plin ../cfg/yolov8s_seg_plin.yaml # plin
   ```
...
1. **如果使用了DetPost硬算子**（如果未使用则忽略此条）
   ...
   - 2）重新进行模型编译
     ...
     ```shell
     icraft compile config/yolov8s_seg_8.toml
     ```
...
3. **修改 运行时的yaml配置文件**：
   ...
   运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在`3_deploy/modelzoo/yolov8_seg/cfg/`

   测试时使用：`yolov8s_seg_test.yaml`
...
4. **编译、执行运行时程序**
   ...
   ```shell
   cd 3_deploy/modelzoo/yolov8_seg
   mkdir build_win
   cd build_win
   cmake ..
   cmake --build . --config Release
   ./Release/yolov8s_seg_psin.exe ../cfg/yolov8s_seg_test.yaml
   ```

   注意：如果使用visualstudio打开sln进行调试，需要右键工程-属性-调试-环境中配置`./Release/yolov8s_seg_psin.exe ../cfg/yolov8s_seg_test.yaml`

5. **使用metrics测试脚本，将保存的测试结果计算出精度指标**

   - metrics测试脚本使用与配置说明：

     [yoloseg_mAP · spiger/benchmark - 码云 - 开源中国 (gitee.com)](https://gitee.com/mxh-spiger/benchmark/tree/master/yoloseg_mAP)
...
| yolov8_seg | input shape     | hard time      | qt_strategy     | 精度             |
| -------------- | --------------- | -------------- | --------------- | ---------------- |
| float          | [1, 3, 640, 640] | -              | -               | box: ap50-95:0.446; mask: ap50-95:0.332 |
| int8           | [1, 3, 640, 640] | 9.71 ms | kld-pc | box: ap50:0.602; ap50-95:0.435; mask: ap50:0.534; ap50-95:0.327 |
| int16          | [1, 3, 640, 640] | 22.29 ms | kld-pc | box: ap50:0.608; ap50-95:0.441; mask: ap50:0.538; ap50-95:0.330 |
