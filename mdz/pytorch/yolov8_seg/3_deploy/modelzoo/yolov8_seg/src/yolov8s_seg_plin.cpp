	...
	auto post_thread = std::thread(
		[&]()
		{
			...
			while (true) {
				...
				Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic> mask_res = std::get<3>(post_results);
				...
				drawTextTwoConer(mask_all, fmt::format("FPS: {:.1f}", fps), MODEL_NAME, color);
				cv::Mat out;
				cvtColor(mask_all, out, cv::COLOR_GRAY2BGR565);
