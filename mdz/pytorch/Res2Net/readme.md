...
- commit_id：01310a5
- weights：https://download.fdwxhb.com/data/04_FMSH-100AI/100AI/04_modelzoo/modelzoo_pub/weights/pytorch/Res2Net/Res2Net_Fbank/best_model/model.pth

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://download.fdwxhb.com/data/04_FMSH-100AI/100AI/04_modelzoo/modelzoo_pub/weights/pytorch/Res2Net/Res2Net_Fbank/best_model/model.pth ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_Res2Net

```shell
# 在此模型根目录
mkdir 0_Res2Net
git clone -b release/0.4.7 https://github.com/yeyupiaoling/AudioClassification-Pytorch.git 0_Res2Net
cd 0_Res2Net
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/Res2Net_8.toml
     ```

     如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）

     其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/Res2Net/imodel

# 3. 仿真

通过配置3_deploy/modelzoo/Res2Net/cfg/中yaml文件的sim字段为True实现模型仿真。
...
2. **执行程序**
   ...
   ```
   python ./Res2Net_psin.py ../cfg/Res2Net.yaml
   ```
...
* 模型测试使用UrbanSound8K数据集的test部分，下载测试数据集请存放于  `\3_deploy\modelzoo\Res2Net\pyrt\dataset\UrbanSound8K`
* `UrbanSound8K`数据集结构为：
```
├── UrbanSound8K
...
3. **修改 运行时的yaml配置文件**：
   ...
   运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在 `3_deploy/modelzoo/Res2Net/cfg/`

   测试时使用：`Res2Net_test.yaml`
...
4. **编译、执行运行时程序**
   ...
   ```
   cd 3_deploy/modelzoo/Res2Net/pyrt
   python ./Res2Net_psin.py ../cfg/Res2Net_test.yaml
   ```
...
| Res2Net | input shape     | hard time      | qt_strategy     | 精度（top-1）             |
| -------------- | --------------- | -------------- | --------------- | ---------------- |
| float          | [1, 398, 80] | -              | -               | 93.241 |
| 8bit           | [1, 398, 80] | 0.923 ms  | [kld-pc] | 92.563  |
| 16bit          | [1, 398, 80] | 1.662ms | [kld-pc] |  92.906|
