...
- code：https://github.com/lucastabelini/PolyLaneNet.git
- branch：master
- commit_id：6155ce2
- weights：https://drive.google.com/open_id=1oyZncVnUB1GRJl5L4oXz50RkcNFM_FFC

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://drive.google.com/open_id=1oyZncVnUB1GRJl5L4oXz50RkcNFM_FFC ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_PolyLaneNet

```shell
# 在此模型根目录
mkdir 0_PolyLaneNet
git clone -b master https://github.com/lucastabelini/PolyLaneNet.git 0_PolyLaneNet
cd 0_PolyLaneNet
git checkout 6155ce2
```
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/PolyLaneNet_8.toml
     ```
 1. 首先根据默认的`config/PolyLaneNet_8.toml`编译模型，其中quantize阶段的`mix_precision = "auto"`
2. 编译完成后，打开`.icraft/logs/PolyLaneNet/PolyLaneNet_mix_config.csv`文件，将倒数第一个`icraft::xir::AvgpoolNode`算子的`dtype`改为`INT16`后保存。
     1. 将`config/PolyLaneNet_8.toml`文件中quantize阶段的混合精度配置为：`mix_precision = ".icraft/logs/PolyLaneNet/PolyLaneNet_mix_config.csv"`。
     2. 再次编译模型，完成编译。
     3. 如果过程顺利，将在`3_deploy/modelzoo/PolyLaneNet/imodel/8`得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）
3. 您也可以直接编译`config/PolyLaneNet_mix.toml`,将在`3_deploy/modelzoo/PolyLaneNet/imodel/mix`得到 icraft model

# 3. 仿真

通过配置3_deploy/modelzoo/PolyLaneNet/cfg/中yaml文件的sim字段为true实现模型仿真。
...
1. **python运行虚拟环境要求与准备**
   ...
   - 在选定的python运行虚拟环境中安装python运行时所需要的依赖包
     ...
     ```
     cd 3_deploy/modelzoo/PolyLaneNet
...
2. **执行程序**
   ...
   ```
   python ./PolyLaneNet_psin.py ../cfg/PolyLaneNet.yaml
   ```
...
| PolyLaneNet | input shape      | hard time | 精度         |
| --------------- | ---------------- | --------- | ------------ |
|   float              | [1, 3, 360, 640] | - |- |
|   8bit             | [1, 3, 360, 640] | 5.786ms |- |
|   16bit              | [1, 3, 360, 640] | 12.705ms |- |
