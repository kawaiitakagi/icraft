...
# io
Set-Location ./3_deploy/modelzoo/dqn/io/
$url_io = "https://download.fdwxhb.com/data/04_FMSH-100AI/100AI/04_modelzoo/modelzoo_pub/deploy/modelzoo/dqn/io/"
& $wget_path $url_io --user $usrname --password $password -r -np -nH -c --reject="index.html*" -P './' --cut-dirs=9
Set-Location $oriModelPath

# onnxruntime
Set-Location ./3_deploy/modelzoo/dqn/onnxruntime/
