...
def enjoy() -> None:  # noqa: C901
    ...
    try:
        with th.no_grad():
            model.eval()
            obs_tensor, vectorized_env = model.base_model.obs_to_tensor(obs)
            th.onnx.export(model, obs_tensor/1, save_path+"\\dqn.onnx", verbose=True)
            print('Onnx model export success, saved in %s' % save_path)

            ftmp_path = R"..\3_deploy\modelzoo\dqn\io\inputs"
            obs_tensor.numpy().astype(np.float32).tofile(ftmp_path + "\\encoder_input.ftmp")
