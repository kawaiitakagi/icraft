...
- branch：main
- commit_id：3af9fa8
- weights：https://dl.fbaipublicfiles.com/detr/detr-r50-e632da11.pth

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://dl.fbaipublicfiles.com/detr/detr-r50-e632da11.pth ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_detr

```shell
# 在此模型根目录
mkdir 0_detr
git clone -b main https://github.com/facebookresearch/detr.git 0_detr
cd 0_detr
git checkout 3af9fa8
```
...
- 1_save.py                              >保存模型，保存好的用于Icraft编译器的模型，会存放在 `/2_compile/fmodel`
  ...
  1. 修改PositionEmbeddingSine的前向输入由原来的tensor_list: NestedTensor类改为Tensor类，方便trace<br>
  2. 修改Transformer的前向输入与计算流程，方便trace<<br>
  3. 修改build_transformer函数的输入参数return_intermediate_dec=True变为return_intermediate_dec=False，保证TransformerDecoder输出最后一层的单个tensor输出<br>
  </div>
  4. 修改模型backbone初始化时候的FrozenBatchNorm2d变为BatchNorm2d
  
  <div style="background-color: #FFFFCC; color: #000000; padding: 10px; border-left: 5px solid #FFA500;">
  另外重要的需要修改的一点是 目前icraft不支持算子bmm，因此需要修改pytorch库中_scaled_dot_product_attention中bmm变为matmul,已验证两者计算结果一致。
  即attn = torch.bmm(q, k.transpose(-2, -1))
  改为attn = torch.matmul(q, k.transpose(-2, -1))
  有两处，函数位置位于Path of your env\Lib\site-packages\torch\nn\functional.py。
  
- 2_save_infer.py                    >用修改后保存的模型做前向推理，验证保存的模型与原模型是否一致

- 3_sim_infer.py              

  运行该脚本需要提前下载对应版本的icraft python api 安装包       

  其用于验证编译后的模型通过仿真后得到的结果与原模型保持一致.
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/detr_8.toml
     ```
  
      如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）
      
      其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/detr/imodel
  
# 3. 仿真

通过配置3_deploy/modelzoo/detr/cfg/中yaml文件的sim字段为True实现模型仿真。
```json
imodel:
   ...
   sim: true
...
1. **编译环境准备**
   ...
   **编译c++程序**
   ...
   ```shell
   #在3.1所需的linux编译环境中
   cd 3_deploy/modelzoo/detr/build_arm
   cmake ..
   make -j
   ```
...
5. **执行程序**
   ...
   ```
   cd /home/fmsh/ModelZoo/modelzoo/detr/build_arm
   chmod 777 *
   ./detr ../cfg/detr.yaml
   ```
...
* 模型测试使用COCO2017数据集的Val部分，测试图片存放于  `3_deploy\modelzoo\yolov5_7.0\io\input`及`3_deploy\modelzoo\yolov5_7.0\io\cocoval_2017.txt`中
* 如需测试其他数据集，需自行将图片存放于`3_deploy\modelzoo\yolov5_7.0\io\input`,并准备对应的input.txt



3. **修改 运行时的yaml配置文件**：

   运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在`3_deploy/modelzoo/detr/cfg/`

   测试时使用：`detr_test.yaml`
...
```shell
cd 3_deploy/modelzoo/detr
mkdir build_win
cd build_win
cmake ..
cmake --build . --config Release
./Release/detr.exe ../cfg/detr_test.yaml
```

注意：如果使用visualstudio打开sln进行调试，需要右键工程-属性-调试-环境中配置`./Release/detr.exe ../cfg/detr_test.yaml`
...
- 本模型使用`./box_mAP/calAP.py` 
- txtDir字段设置为片上系统预测结果存放路径
- dtJson字段设置为预测结果生成txt的存放路径



# 6. 模型性能记录

| detr | input shape     | hard time      | qt_strategy     | 精度             |
| -------------- | --------------- | -------------- | --------------- | ---------------- |
| float          | [1, 3, 640, 640] | -              | -               | ap50:0.558,ap50-95:0.365 |
| int8           | [1, 3, 640, 640] | 41.08815ms | kld-pc | ap50:0.276, ap50-95:0.129 |
| int16          | [1, 3, 640, 640] | 94.3828ms | kld-pc | ap50:0.532, ap50-95:0.343 |
