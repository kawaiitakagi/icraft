...
- branch：master
- commit_id：27e081e
- weights：https://download.fdwxhb.com/data/04_FMSH-100AI/100AI/04_modelzoo/modelzoo_pub/weights/pytorch/sac
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_sac

```shell
# 在此模型根目录
mkdir 0_sac
git clone -b master https://github.com/DLR-RM/rl-baselines3-zoo.git 0_sac
cd 0_sac
...
- 1_save.py    >保存模型与ONNX输入，保存为onnx模型，存放在：`\3_deploy\modelzoo\sac\imodel` , 输出存放在：`\3_deploy\modelzoo\sac\io`
...
2. **版本依赖下载**
   ...
   主要版本依赖已存放在`\3_deploy\modelzoo\sac\onnxruntime`
...
3. **编译c++程序**
   ...
   ```shell
   #在3.1所需的linux编译环境中
   cd 3_deploy/modelzoo/sac/build_arm
   ...
   模型输入均在 `3_deploy/modelzoo/sac/io`中，可根据需要进行替换，生成方式如下：
   
   ```
   # 仅测试时间，python中随机生成输出
   obs_onnx = torch.randn(1,2,64,64,dtype=torch.float32)
   obs_onnx.tofile("encoder_input.ftmp")
   # 最后手动放入对应3_deploy/modelzoo_v3.0.1/sac/io/inputs/sac目录中
   ```
   
5. **执行程序**

   运行前需要手动把3_deploy\modelzoo\sac\onnxruntime\lib\aarch64下的libonnxruntime.so, libonnxruntime.so.1.17.1复制到运行环境中，例如 usr/lib下, 然后执行：

   ```
   cd /home/fmsh/ModelZoo/modelzoo/sac/build_arm
   chmod 777 *
   ./ort_run
   ```
