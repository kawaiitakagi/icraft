...
- 0_ERes2Net    >模型原始工程，需要自行下载
- weights              >存放原始权重，需要自行下载
- 1_scripts            >若干脚本，用于保存Icraft编译器需要的模型、编译后仿真等功能
- 2_compile          >Icraft编译器编译模型时所需要的文件
- 3_deploy            >将Icraft编译器编译出的模型部署到硬件时需要的部署工程

# 1. python工程准备

## 1. **模型来源：**

- code：https://gitee.com/yeyupiaoling/VoiceprintRecognition-Pytorch.git
- branch：release/1.0.5
- commit_id：69e1cef
- weights：https://download.fdwxhb.com/data/04_FMSH-100AI/100AI/04_modelzoo/modelzoo_pub/weights/pytorch/ERes2Net/ERes2Net_TSTP_Fbank/best_model/model.pth

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://download.fdwxhb.com/data/04_FMSH-100AI/100AI/04_modelzoo/modelzoo_pub/weights/pytorch/ERes2Net/ERes2Net_TSTP_Fbank/best_model/model.pth ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_ERes2Net

```shell
# 在此模型根目录
mkdir 0_ERes2Net
git clone -b release/1.0.5 https://gitee.com/yeyupiaoling/VoiceprintRecognition-Pytorch.git 0_ERes2Net
cd 0_ERes2Net
git checkout 69e1cef
```
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/ERes2Net_8.toml
     ```

     如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）

     其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/ERes2Net/imodel

# 3. 仿真

通过配置3_deploy/modelzoo/ERes2Net/cfg/中yaml文件的sim字段为True实现模型仿真。
...
2. **执行程序**
   ...
   ```
   python ./ERes2Net_psin.py ../cfg/ERes2Net.yaml
   ```
...
1. **如果使用了detpost硬算子**（本模型未使用，请忽略此条）
   ...
   - 2）重新进行模型编译
     ...
     ```shell
     icraft compile config/EcapaTDNN_8.toml
     ```

     由于测精度程序由上位机控制，生成的imodel不用再复制到板子中
2. **数据集准备**

* 测试数据集下载：[cn-celeb-test.zip](https://aistudio.baidu.com/datasetdetail/233361)
* 模型测试使用cn-celeb数据集的test部分，下载测试数据集请存放于  `\3_deploy\modelzoo\ERes2Net\pyrt\dataset\cn-celeb-test`
* `cn-celeb-test`数据集结构为：
```
├── cn-celeb-test
│   ├── enroll_list.txt
│   ├── trials_list.txt
...
│   │   ├── id00800-singing-01-002.flac
│   │   ├── id00800-singing-01-003.flac
│   │   ├── ...
│   │   └── id00999-singing-02-006.flac
```
* 运行`0_ERes2Net/extract_features.py`,给定配置文件后获得处理后的音频输入特征，存放在`cn-celeb-test/enroll_list_features.txt`、`cn-celeb-test/trials_list_features.txt`和`dataset/features`
```
├── features
│   ├── 0
│   ├── 1
...
3. **修改 运行时的yaml配置文件**：
   ...
   运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在 `3_deploy/modelzoo/ERes2Net/cfg/`

   测试时使用：`ERes2Net_test.yaml`
...
4. **编译、执行运行时程序**
   ...
   ```
   cd 3_deploy/modelzoo/ERes2Net/pyrt
   python ./ERes2Net_test.py ../cfg/ERes2Net_test.yaml
   ```
...
| ERes2Net | input shape     | hard time      | qt_strategy     | 精度             |
| -------------- | --------------- | -------------- | --------------- | ---------------- |
| float          | [1, 365, 80] | -              | -               | <br>EER:0.13567</br> <br>MinDCF:0.63016</br> |
| 8bit           |  [1, 365, 80] | 10.84 ms  | [kld-pc] |<br>EER:0.16007</br> <br>MinDCF:0.68218</br>  |
| 16bit          |  [1, 365, 80] | 23.895ms | [kld-pc] |  <br>EER:0.14276</br> <br>MinDCF:0.66264</br>|
