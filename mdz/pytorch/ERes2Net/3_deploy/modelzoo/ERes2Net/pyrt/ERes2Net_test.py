...
def main(config_path) -> None:
    ...
    for batch_id, (audio_features, label, input_lens) in enumerate(
        ...
        # save input_feature
        # np.savetxt('./input_tmp/input_'+str(batch_id)+'.txt',audio_features[0],fmt="%.6f")
        # save output_feature
        # np.savetxt('./output_tmp/output_'+str(batch_id)+'.txt',feature,fmt="%.6f")
        # 存放特征
    ...
    print('trials_labels =',trials_labels,trials_labels.shape)
    ...
    if save:
        ...
        # save test results
        ERR_PATH = R'./ERes2Net_metrics.log'
