...
- code：https://github.com/WongKinYiu/yolov7.git
- branch：pose
- commit_id：cad7aca
- weights：https://github.com/WongKinYiu/yolov7/releases/download/v0.1/yolov7-w6-pose.pt

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://github.com/WongKinYiu/yolov7/releases/download/v0.1/yolov7-w6-pose.pt ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_yolov7_pose

```shell
# 在此模型根目录
mkdir 0_yolov7_pose
git clone -b pose https://github.com/WongKinYiu/yolov7.git 0_yolov7_pose
cd 0_yolov7_pose
git checkout cad7aca
```
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/yolov7-w6-pose-deploy-8.toml
     ```
  
      如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）
  
      其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/yolov7_pose/imodel
  
# 3. 仿真

通过配置3_deploy/modelzoo/yolov7_pose/cfg/中yaml文件的sim字段为True实现模型仿真。
```json
imodel:
   ...
   sim: true
...
**编译c++程序**
   ...
   ```shell
   #在3.1所需的linux编译环境中
   cd 3_deploy/modelzoo/yolov7_pose/build_arm
   cmake ..
   make -j
   ```
...
5. **执行程序**
   ...
   ```
   cd /home/fmsh/ModelZoo/modelzoo/yolov7_pose/build_arm
   chmod 777 *
   ./yolov7-pose-w6 ../cfg/yolov7-w6-pose.yaml
   ```
...
3. **修改 运行时的yaml配置文件**：
   ...
   运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在`3_deploy/modelzoo/yolov7_pose/cfg/`

   测试时使用：`yolov7-w6-pose_test.yaml`
...
```shell
cd 3_deploy/modelzoo/yolov7_pose
mkdir build_win
cd build_win
cmake ..
cmake --build . --config Release
./Release/yolov7-pose-w6.exe ../cfg/yolov7-w6-pose_test.yaml
```

注意：如果使用visualstudio打开sln进行调试，需要右键工程-属性-调试-环境中配置`./Release/yolov7-pose-w6.exe ../cfg/yolov7-w6-pose_test.yaml`
...
| yolov7_pose | input shape     | hard time      | qt_strategy     | 精度             |
| -------------- | --------------- | -------------- | --------------- | ---------------- |
| float          | [1, 3, 640, 640] | -              | -               | 官方未列出 |
| int8           | [1, 3, 640, 640] | 24.2247ms | kld-pt | ap50:0.834; ap50-95:0.491 |
| int16          | [1, 3, 640, 640] | 55.399ms | kld-pt | ap50:0.869; ap50-95:0.651 |
