...
#include "postprocess_yolov7_pose.hpp"
...
int main(int argc, char* argv[])
{
	...
	// YAML::Node config = YAML::LoadFile("../cfg/yolov7-w6-pose.yaml");
	// icraft模型部署相关参数配置
		...
		// 计时
		 ...
		 if (netinfo.DetPost_on) {
		 	post_detpost_hard(outputs, img,
		 		netinfo, conf, iou_thresh, MULTILABEL, fpga_nms, N_CLASS, NOP, ANCHORS,LABELS,
		 		show,save,resRoot,name,device,run_sim);
		 }
		 else {
		 	post_detpost_soft(outputs, img, LABELS,ANCHORS,netinfo,
		 		N_CLASS,NOP, conf,iou_thresh,show,save,resRoot,name);
		 }
