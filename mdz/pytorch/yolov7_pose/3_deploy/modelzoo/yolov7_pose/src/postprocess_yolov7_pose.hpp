...
void get_cls_bbox(std::vector<int>& id_list, std::vector<float>& socre_list, std::vector<cv::Rect2f>& box_list, std::vector<std::vector<std::vector<float>>>& key_point_list, T* tensor_data, int base_addr,
    ...
    std::vector<float> anchor, int N_CLASS,int NOP, float THR_F, bool MULTILABEL) {
    /*int NOP = (N_CLASS - 1) / 3;*/
    if (!MULTILABEL) {
        ...
        if (realscore > THR_F) {
            ...
            T* kpt_ptr_start = tensor_data + base_addr + 5 + 1;
            std::vector<std::vector<float>> one_obj_kpt;
            for (int k = 0; k < NOP; ++k) {
                std::vector<float> one_kpt;
                float ori_x = kpt_ptr_start[k * 3] * SCALE;
                float ori_y = kpt_ptr_start[k * 3 + 1] * SCALE;
                float kpt_score = kpt_ptr_start[k * 3 + 2] * SCALE;
                float kpt_x = (ori_x * 2. - 0.5 + grid.location_x) * stride;
                float kpt_y = (ori_y * 2. - 0.5 + grid.location_y) * stride;
        ...
        for (size_t cls_idx = 0; cls_idx < N_CLASS; cls_idx++) {
            ...
            auto realscore = _score_ * _prob_;
            if (realscore > THR_F) {
                //auto bbox = this->getBbox(tensor_data, norm, base_addr, location_x, location_y, stride, anchor);
                std::vector<float> xywh = sigmoid(tensor_data, SCALE, base_addr, 4);
    ...
    for (size_t i = 0; i < output_tensors.size(); i++) {
        ...
        if (output_tensors_bits == 16) {
            ...
            for (size_t obj = 0; obj < obj_num; obj++) {
                ...
                get_cls_bbox(id_list, socre_list, box_list, key_point_list, tensor_data, base_addr, grid, netinfo.o_scale[i], stride[i],ANCHORS[i][grid.anchor_index], N_CLASS, NOP, conf, MULTILABEL);
            }
        }
        else {
...
void post_detpost_soft(const std::vector<Tensor>& output_tensors, PicPre& img, std::vector<std::string>& LABELS,
    std::vector<std::vector<std::vector<float>>>& ANCHORS, NetInfo& netinfo, int N_CLASS, int NOP,float conf, float iou_thresh,
        ...
        for (size_t h = 0; h < _H; h++) {
            ...
            for (size_t w = 0; w < _W; w++) {
                ...
                for (size_t anchor_index = 0; anchor_index < ANCHORS[yolo].size(); anchor_index++) {
                    ...
                    auto boxPtr = tensor_data + h * _W * (N_CLASS + 5 + 3 * NOP) * ANCHORS[yolo].size()
                        + w * (N_CLASS + 5 + 3 * NOP) * ANCHORS[yolo].size() + anchor_index * (N_CLASS + 5 + 3 * NOP);
                    ...
                    if(realscore >conf){
                        ...
                        for (int k = 0; k < NOP; ++k) {
                            ...
                            float kpt_x = (ori_x * 2. - 0.5 + w) * one_head_stride;
                            float kpt_y = (ori_y * 2. - 0.5 + h) * one_head_stride;
                            float kpt_prob = sigmoid(kpt_score);
                            one_kpt = { kpt_x, kpt_y, kpt_prob };
                            one_obj_kpt.emplace_back(one_kpt);
                        }
    ...
    std::vector<std::tuple<int, float, cv::Rect2f, std::vector<std::vector<float>>>> nms_res = nms_soft(id_list, socre_list, box_list, key_point_list, iou_thresh);   // 后处理 之 NMS
