...
- 0_JL-DCF-pytorch-master    >模型原始工程，需要自行下载
...
- code：https://github.com/jiangyao-scu/JL-DCF-pytorch
- master
- d7bcb71
- weights：https://drive.google.com/open?id=12u37yz-031unDPJoKaZ0goK8BtPP-6Cj

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://drive.google.com/open?id=12u37yz-031unDPJoKaZ0goK8BtPP-6Cj ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_JL-DCF-pytorch-master

```shell
# 在此模型根目录
mkdir 0_JL-DCF-pytorch-master
git clone -b master https://github.com/jiangyao-scu/JL-DCF-pytorch.git 0_JL-DCF-pytorch-master
cd 0_JL-DCF-pytorch-master
git checkout d7bcb71
```
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/JLDCF.toml
     ```
...
1. **python运行虚拟环境要求与准备**
   ...
   - 安装一些便于开发pyrt的依赖包
     ...
     ```
     cd 3_deploy/Deps/modelzoo/JL-DCF
     pip install -e .
     ```
...
2. **执行程序**
   ...
   ```
   python ./JL-DCF_psin.py ../cfg/JL-DCF.yaml
   ```
...
| JL-DCF | input shape     | hard time      | qt_strategy     | 精度             |
| -------------- | --------------- | -------------- | --------------- | ---------------- |
| float          | [1, 3, 320, 320], [1, 3, 320, 320] | - | - | SSIM:0.855; Fβ:0.894; MAE:0.108 |
| 8bit           | [1, 3, 320, 320], [1, 3, 320, 320] | 128.7367ms | kld-pc | SSIM:0.0.857; Fβ:0.892; MAE:0.107 |
| 16bit          | [1, 3, 320, 320], [1, 3, 320, 320] | 372.1721ms | kld-pc | SSIM:0.0.859; Fβ:0.893; MAE:0.105 |
