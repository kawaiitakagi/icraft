...
1）根据模型来源中的地址：https://github.com/SwinTransformer/storage/releases/download/v1.0.0/swin_tiny_patch4_window7_224.pth ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_swin

```shell
# 在此模型根目录
mkdir 0_swin
git clone -b master https://github.com/microsoft/Swin-Transformer.git 0_swin
cd 0_swin
git checkout f82860b
```
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/swin_tiny_8.toml
     ```

     如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）

     其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/swin/imodel

# 3. 仿真

通过配置3_deploy/modelzoo/swin/cfg/中yaml文件的sim字段为True实现模型仿真。
...
2. **执行程序**
      ...
      ```
      python ./swin_psin.py ../cfg/swin.yaml
      ```
...
* 模型测试使用imagenet2012val的前5000张图片，图片下载完成后将前5000张图片放入任意路径文件夹中，将3_deploy/modelzoo/swin/cfg/中swin_test.yaml文件的dataset::dir字段改为图片所在的路径，测试所需要的label和list都已经提前准备好放置在io文件夹中了，且swin_test.yaml已经正确设置了
* 如需测试其他数据集，需自行将图片存放于 `3_deploy\modelzoo\swin\io\input`,并准备对应的input.txt及label信息

3. **修改 运行时的yaml配置文件**：

运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在 `3_deploy/modelzoo/swin/cfg/`

测试时使用：`swin_test.yaml`
...
在成功部署模型的python运行虚拟环境中运行下列指令，即可完成精度测试

```
python ./swin_psin.py ../cfg/swin_test.yaml
```
...
| swin | input shape      | hard time       | qt_strategy      | 精度              |
| --------------- | ---------------- | --------------- | ---------------- | ----------------- |
| float           | [1, 3, 224, 224] | -               | -                | top1:81.2%, top5:95.3% |
| int8            | [1, 3, 224, 224] | 15.5495ms | ['ems-pc'] | [{'top1': 70.84%}, {'top5': 89.22%}] |
| int16           | [1, 3, 224, 224] | 38.0375ms | ['ems-pc'] | [{'top1': 74.80%}, {'top5': 92.08%}] |
