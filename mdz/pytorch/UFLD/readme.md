...
- code：https://github.com/Turoad/lanedet.git
- branch：main
- commit_id：35665fc
- weights：https://github.com/Turoad/lanedet/releases/download/1.0/ufld_r18_tusimple.zip

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://github.com/Turoad/lanedet/releases/download/1.0/ufld_r18_tusimple.zip ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_UFLD

```shell
# 在此模型根目录
mkdir 0_UFLD
git clone -b main https://github.com/Turoad/lanedet.git 0_UFLD
cd 0_UFLD
git checkout 35665fc
```
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/UFLD_8.toml
     ```

     如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）

     其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/UFLD/imodel

# 3. 仿真

通过配置3_deploy/modelzoo/UFLD/cfg/中yaml文件的sim字段为true实现模型仿真。
...
2. **执行程序**
   ...
   ```
   python ./UFLD_psin.py ../cfg/UFLD.yaml
   ```
...
| UFLD | input shape      | hard time | 精度         |
| --------------- | ---------------- | --------- | ------------ |
|   float              | [1, 3, 288, 800] | - |- |
|   8bit             | [1, 3, 288, 800] | 10.619ms |- |
|   16bit              | [1, 3, 288, 800] | 21.160ms |- |
