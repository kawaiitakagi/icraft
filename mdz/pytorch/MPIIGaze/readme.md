...
- code：https://github.com/hysts/pytorch_mpiigaze.git
- branch：master
- commit_id：2a31a6c
- weights：https://github.com/hysts/pytorch_mpiigaze_demo/releases/download/v0.1.0/mpiigaze_resnet_preact.pth

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://github.com/hysts/pytorch_mpiigaze_demo/releases/download/v0.1.0/mpiigaze_resnet_preact.pth ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_MPIIGaze

```shell
# 在此模型根目录
mkdir 0_MPIIGaze
git clone -b master https://github.com/hysts/pytorch_mpiigaze 0_MPIIGaze
cd 0_MPIIGaze
git checkout 2a31a6c
```

3）根据源码readme构建环境，进入0_MPIIGaze/，打开wsl环境，运行`bash scripts/download_dlib_model.sh`下载`shape_predictor_68_face_landmarks.dat`，下载好后该文件会放在0_MPIIGaze/data/dlib/目录下。

4）将1_scripts中的脚本复制到0_MPIIGaze路径下，在0_MPIIGaze路径下执行保存模型脚本，会将保存好的模型放在2_compile/fmodel路径下。
...
- 1_save.py                              >保存模型，保存好的用于Icraft编译器的模型，会存放在 `/2_compile/fmodel`
  ...
  <div style="background-color: #FFFFCC; color: #000000; padding: 10px; border-left: 5px solid #FFA500;">
  保存模型时的修改点:<br>
    原模型输入为两张灰度眼图拼接后的结果[2,1,36,60]，但Icraft generate不支持此类b=2的情况，因此将输入拆分为[1,1,36,60]，左右眼图分别跑两次前向。</div>
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/MPIIGaze_8.toml
     ```

     如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）

     其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/MPIIGaze/imodel

# 3. 仿真

通过配置3_deploy/modelzoo/MPIIGaze/cfg/中yaml文件的sim字段为True实现模型仿真。
...
1. **python运行虚拟环境要求与准备**
   ...
   - 在0_MPIIGaze的python运行虚拟环境中icraft的python安装包：`pip install icraft-3.x.x-cp38-none-win_amd64.whl`
   ...
   - 运行工程前，需手动将0_MPIIGaze/data/dlib路径下的shape_predictor_68_face_landmarks.dat文件拷贝到3_deploy/modelzoo/MPIIGaze/pyrt/data/dlib路径下。

   - 在选定的python运行虚拟环境中安装python运行时所需要的依赖包

     ```
     cd 3_deploy/modelzoo/MPIIGaze
   ...
   - 安装一些便于开发pyrt的依赖包
     ...
     <div style="background-color: #FFFFCC; color: #000000; padding: 10px; border-left: 5px solid #FFA500;">
     1.由于python运行时代码在引入依赖包时候已经通过sys.path.append(R"../../../Deps/modelzoo")将一些便于开发pyrt的依赖包加入到了系统路径下，因此可以不用执行该步骤即可直接运行2.执行程序。这里提供额外的[pip install -e .]安装方式是为了在python运行时代码时候方便进行pyrtutils的依赖跳转。

2. **执行程序**

   （1）**MPIIGaze_psin.py部署的是源码中的demo.py**，即从相机拍摄的图像=》dlib人脸&关键点检测=》视线估计网络的过程；**运行前需确保已安装好0_MPIIGaze readme中所需环境，该工程依赖于0_MPIIGaze中的gaze_estimation**，其Icraft-python运行时工程执行方法如下：
   
   ```
   python ./MPIIGaze_psin.py ../cfg/MPIIGaze.yaml
   ```
   
   可在io/output中查看结果。
   
   （2）**MPIIGaze_psin_test.py部署的是源码中的evaluate.py**，即将MPIIGaze数据集中裁剪好的人眼图像&pose直接输入视线估计网络，输出视线估计结果&精度指标；本工程中的io文件夹中提供预处理好的p00的测试集。如需测试其它人物，**可自行下载MPIIGaze数据集，并按照0_MPIIGaze中的readme中数据预处理的方法先处理原始数据集**。
   
   MPIIGaze数据集下载&预处理命令如下：
   ...
   Icraft-python运行时工程执行方法如下：
   
   ```
   python ./MPIIGaze_psin_test.py ../cfg/MPIIGaze_psin.yaml
   ```
   
   可在io/output中查看精度测试结果。
...
1. **重新编译模型**
   ...
   - 1）由于源码工程未提供精度测试的预训练模型，可运行0_MPIIGaze/train.py训练模型，也可使用已重训好的权重`checkpoint_0040.pth`

   - 2）使用checkpoint_0040.pth权重重新运行1_save.py，导出模型
...
- 测试数据集下载：http://datasets.d2.mpi-inf.mpg.de/MPIIGaze/MPIIGaze.tar.gz
- 模型测试使用MPIIGaze数据集的`p00`作为测试集，将下载下来的数据集MPIIGaze存放于  `3_deploy\modelzoo\MPIIGaze\io\datasets`中，并解压到当前文件夹

3. **修改 运行时的yaml配置文件**：

   运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在 `cfg/MPIIGaze_test.yaml`

   测试时使用：`python ./MPIIGaze_psin_test.py ../cfg/MPIIGaze_test.yaml`
...
```
cd 3_deploy/modelzoo/MPIIGaze/pyrt
python ./MPIIGaze_psin_test.py ../cfg/MPIIGaze_test.yaml
```

程序结束自动计算出精度指标angle_error，并记录在MPIIGaze_metrics.log中



# 6. 模型性能记录

| MPIIGaze | input shape      | hard time       | qt_strategy      | angle_error  |
| --------------- | ---------------- | --------------- | ---------------- | ----------------- |
| float           | [1, 1, 36, 60] | -               | -               | 5.24° |
| int8            | [1, 1, 36, 60] | 0.0965ms  | null-pt | 4.76° |
| int16           | [1, 1, 36, 60] | 0.1275ms | null-pt | 4.78° |
