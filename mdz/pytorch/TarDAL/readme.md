...
- main
- 6a9edd7
- weights：https://github.com/JinyuanLiu-CV/TarDAL/releases/download/v1.0.0/tardal-dt.pth

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://github.com/JinyuanLiu-CV/TarDAL/releases/download/v1.0.0/tardal-dt.pth ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_TarDAL-main

```shell
# 在此模型根目录
mkdir 0_TarDAL-main
git clone -b main https://github.com/JinyuanLiu-CV/TarDAL.git 0_TarDAL-main
cd 0_TarDAL-main
git checkout 6a9edd7
```

创建runs文件夹并下载m3fd数据集 //pan.baidu.com/s/1GoJrrl_mn2HNQVDSUdPCrw?pwd=M3FD 到 0_TarDAL-main/data/m3fd/ 路径下
```shell
cd 0_TarDAL-main
mkdir runs
cd runs
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/tardal-dt.toml
     ```
...
2. **执行程序**
   ...
   ```
   python ./TarDAL_psin.py ../cfg/TarDAL.yaml
   ```
...
| TarDAL | input shape     | hard time      | qt_strategy     | 精度             |
| -------------- | --------------- | -------------- | --------------- | ---------------- |
| float          | [1, 1, 768, 1024], [1, 1, 768, 1024] | - | - | MI:2.356; EN:6.889; SD:43.323; PSNR:12.270; SSIM:0.752 |
| 8bit           | [1, 1, 768, 1024], [1, 1, 768, 1024] | 109.868ms | kld-pc | MI:2.349; EN:5.009; SD:43.704; PSNR:12.171; SSIM:0.728 |
| 16bit          | [1, 1, 768, 1024], [1, 1, 768, 1024] | 225.6657ms | kld-pc | None |
