...
- code：https://github.com/open-mmlab/mmsegmentation.git
- branch：main
- commit_id：1471d1e
...
1）根据模型来源中的地址：https://download.openmmlab.com/mmsegmentation/v0.5/pspnet/pspnet_r50-d8_512x1024_40k_cityscapes/pspnet_r50-d8_512x1024_40k_cityscapes_20200605_003338-2966598c.pth ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_MMPSPNet

```shell
# 在此模型根目录
mkdir 0_MMPSPNet
git clone -b main https://github.com/open-mmlab/mmsegmentation.git 0_MMPSPNet
cd 0_MMPSPNet
git checkout 1471d1e
```
...
- 1_save.py                              >保存模型，保存好的用于Icraft编译器的模型，会存放在 `/2_compile/fmodel`
  ...
  <div style="background-color: #FFFFCC; color: #000000; padding: 10px; border-left: 5px solid #FFA500;">
  保存模型时的修改点：<br>
  1. 去除模型前处理以及auxiliary_head<br>
  2. 将dropout2d置空<br>
  3. 将psp_head中upsample操作拆分成多次进行，避免上采样倍数过高<br>
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/MMPSPNet_8.toml
     ```
  
      如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）
  
      其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/mmpspnet/imodel
  
# 3. 仿真

通过配置3_deploy/modelzoo/MMPSPNet/cfg/中yaml文件的sim字段为True实现模型仿真。
```json
imodel:
   ...
   sim: true
...
**编译c++程序**
   ...
   ```shell
   #在3.1所需的linux编译环境中
   cd 3_deploy/modelzoo/MMPSPNet/build_arm
   cmake ..
   make -j
   ```
...
5. **执行程序**
   ...
   ```
   cd /home/fmsh/ModelZoo/modelzoo/MMPSPNet/build_arm
   chmod 777 *
   ./MMPSPNet_psin ../cfg/MMPSPNet.yaml
   ```
...
1. **数据集准备**
   ...
   * 模型测试使用cityscapes数据集，测试图片存放于  `3_deploy\modelzoo\MMPSPNet\io\cityscapes\leftImg8bit`文件夹下，对应的list文件存放`3_deploy\modelzoo\MMPSPNet\io\cityscapes\valImages.txt`中
   * 如需测试其他数据集，需自行将图片存放于`3_deploy\modelzoo\MMPSPNet\io\input`,并准备对应的input.txt



2. **修改运行时的yaml配置文件**：

   运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在`3_deploy/modelzoo/MMPSPNet/cfg/`

   测试时使用：`MMPSPNet_test.yaml`
...
3. **编译、执行运行时程序**
   ...
   ```shell
   cd 3_deploy/modelzoo/MMPSPNet
   mkdir build_win
   cd build_win
   cmake ..
   cmake --build . --config Release
   ./Release/MMPSPNet_psin.exe ../cfg/MMPSPNet_test.yaml
   ```

   注意：如果使用visualstudio打开sln进行调试，需要右键工程-属性-调试-命令参数中配置` ../cfg/MMPSPNet_test.yaml`
...
| mmpspnet | input shape     | hard time      | qt_strategy     | 精度             |
| -------------- | --------------- | -------------- | --------------- | ---------------- |
| float          | [1, 3, 1024, 2048] | -              | -               | mAcc:84.86; mIoU:77.92; allAcc:95.99 |
| 8bit           | [1, 3, 1024, 2048] | 540.605ms | kld-pt | mAcc:83.13; mIoU:76.90; allAcc:95.79 |
| 16bit          | [1, 3, 1024, 2048] | 1401.880ms | kld-pt | mAcc:84.31; mIoU:77.61; allAcc:95.88 |
