...
- code：https://github.com/meituan/YOLOv6.git
- branch：0.2.0
- commit_id：7259130
- weights：https://github.com/meituan/YOLOv6/releases/download/0.2.0/yolov6s_v2_reopt.pt

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://github.com/meituan/YOLOv6/releases/download/0.2.0/yolov6s_v2_reopt.pt ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_yolov6

```shell
# 在此模型根目录
mkdir 0_yolov6
git clone -b 0.2.0 https://github.com/meituan/YOLOv6.git 0_yolov6
cd 0_yolov6
git checkout 7259130
```
...
python 1_save.py
```
注：在`0_yolov6/requirements.txt`中的pytorch_quantization>=2.1.1配置较复杂，若仅运行1_scripts中的脚本，可不安装此项依赖。
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/yolov6_s_v2_reopt_8.toml
     ```
  
      如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）
  
      其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/yolov6/imodel
  
# 3. 仿真

通过配置3_deploy/modelzoo/yolov6/cfg/中yaml文件的sim字段为True实现模型仿真。
```json
imodel:
   ...
   sim: true
...
**编译c++程序**
   ...
   ```shell
   #在3.1所需的linux编译环境中
   cd 3_deploy/modelzoo/yolov6/build_arm
   cmake ..
   make -j
   ```
...
5. **执行程序**
   ...
   ```
   cd /home/fmsh/ModelZoo/modelzoo/yolov6/build_arm
   chmod 777 *
   ./yolov6 ../cfg/yolov6.yaml
   ```
...
* 模型测试使用COCO2017数据集的Val部分，测试图片存放于  `3_deploy\modelzoo\yolov6\io\input`及`3_deploy\modelzoo\yolov6\io\cocoval_2017.txt`中
* 如需测试其他数据集，需自行将图片存放于`3_deploy\modelzoo\yolov6\io\input`,并准备对应的input.txt



3. **修改 运行时的yaml配置文件**：

   运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在`3_deploy/modelzoo/yolov6/cfg/`

   测试时使用：`yolov6_test.yaml`
...
```shell
cd 3_deploy/modelzoo/yolov6
mkdir build_win
cd build_win
cmake ..
cmake --build . --config Release
./Release/yolov6.exe ../cfg/yolov6_test.yaml
```

注意：如果使用visualstudio打开sln进行调试，需要右键工程-属性-调试-环境中配置`./Release/yolov6.exe ../cfg/yolov6_test.yaml`
...
| yolov6 | input shape     | hard time      | qt_strategy     | 精度             |
| -------------- | --------------- | -------------- | --------------- | ---------------- |
| float          | [1, 3, 640, 640] | -              | -               | ap50-90:43.3 |
| 8bit           | [1, 3, 640, 640] | 5.97849ms  | kld-pt | ap50:58.0,ap50-90:40.7  |
| 16bit          | [1, 3, 640, 640] | 14.116316ms | kld-pt | ap50:58.7,ap50-90:42.0 |
