...
int main(int argc, char* argv[])
{
    ...
    if (task == "long_term_forecast") {
        ...
        for (auto name : namevector) {
            ...
            for (int i = (dec_in - 1)* seq_len; i < seq_len * dec_in; i += 1) {
                input_seq.push_back(*(input_ptr + i));
            }

            //forward
            auto output_tensors = session.forward({ x_tensor,pos_emb_tensor, hx_tensor });
    ...
    if (task == "exp_classification") {
        //固定除了图片以外的其他输入
        auto pos_emb_tensor = hostbackend::utils::Ftmp2Tensor("../io/cls/input/pos_emb.ftmp", network.inputs()[1].tensorType());
        auto hx_tensor = hostbackend::utils::Ftmp2Tensor("../io/cls/input/hx.ftmp", network.inputs()[2].tensorType());
        // 统计图片数量
        int index = 0;
        auto namevector = toVector(imgList);
        int totalnum = namevector.size();
        ...
        for (auto name : namevector) {
            ...
            //    input_seq.push_back(*(input_ptr + i));
            //}
            for (int i = (dec_in - 2) * seq_len; i < seq_len * (dec_in-1); i += 1) {
                input_seq.push_back(*(input_ptr + i));
            }
            int show_w = (((int)input_seq.size() * 4) / 100 + 1) * 100;
            int show_h = show_w * 0.75;
