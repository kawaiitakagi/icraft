...
- code：https://gitee.com/mxh-spiger/custom_model/tree/master/0_edsr
- branch：master
- commit_id：b16a6a0
- weights：https://cv.snu.ac.kr/research/EDSR/model_pytorch.tar

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://cv.snu.ac.kr/research/EDSR/model_pytorch.tar ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_edsr

```shell
# 在此模型根目录
mkdir 0_edsr
git clone -b master https://github.com/sanghyun-son/EDSR-PyTorch 0_edsr
cd 0_edsr
git checkout 8dba558
```
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/edsr.toml
     ```

     如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）

     其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/edsr/imodel

# 3. 仿真

通过配置3_deploy/modelzoo/edsr/cfg/中yaml文件的sim字段为True实现模型仿真。
...
1. **python运行虚拟环境要求与准备**
   ...
   - 在选定的python运行虚拟环境中安装python运行时所需要的依赖包
     ...
     ```
     cd 3_deploy/modelzoo/yolov5_7.0
...
2. **执行程序**
   ...
   ```
   python ./edsr_psin.py ../cfg/edsr.yaml
   ```
...
* 测试数据集下载：[B100](https://paperswithcode.com/dataset/bsd100)
...
3. **修改 运行时的yaml配置文件**：
   ...
   运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在 `3_deploy/modelzoo/edsr/cfg/`

   测试时使用：`edsr_test.yaml`
...
```
cd 3_deploy/modelzoo/edsr/pyrt
python ./edsr_psin.py ../cfg/edsr_test.yaml
```
...
```shell
cd 3_deploy/modelzoo/edsr
mkdir build_win
cd build_win
cmake ..
cmake --build . --config Release
./Release/edsr_psin.exe ../cfg/edsr_test.yaml
```

注意：如果使用visualstudio打开sln进行调试，需要右键工程-属性-调试-环境中配置 `./Release/edsr_psin.exe ../cfg/edsr_test.yaml`
...
- 本模型使用 `./PSNR/PSNR_edge_processed.py`
- txtDir字段设置为片上系统预测结果存放路径
- dtJson字段设置为预测结果生成txt的存放路径

# 6. 模型性能记录

| edsr            | input shape      | hard time                                  | qt_strategy      | psnr                         |
| --------------- | ---------------- | -------------------------------------------| ---------------- | ---------------------------- |
| float           | [1, 3, 160, 240] | -                                          | -                | edsr 31.863; edsr_big 31.937 |
| int8            | [1, 3, 160, 240] | edsr_glue 15.432ms; edsr_big_gelu 15.433ms | null-pt          | edsr 30.746; edsr_big 29.418 |
| int16           | [1, 3, 160, 240] | edsr_gelu 30.344ms; edsr_big_gelu 30.343ms | null-pt          | edsr 31.862; edsr_big 29.477 |
| mix             | [1, 3, 160, 240] | -                                          | null-pt          | edsr 30.791; edsr_big 29.432 |

# 7. 备注

- 上述精度测试所用图片为B100中大小为160*240的图片，所以上述精度结果为77张图片的psnr值。
- 本工程共提供两个模型，其中edsr_big_glue是为了提升浮点精度而设计，如若考虑模型量化部署建议使用edsr_glue.