...
#include "PicPre_yolov5_hrsid.hpp"
#include "postprocess_yolov5_hrsid.hpp"
...
int main(int argc, char* argv[])
{
	...
	// YAML::Node config = YAML::LoadFile("../cfg/yolov5_hrsid.yaml");
	// icraft模型部署相关参数配置
	...
	for (auto name : namevector) {
		...
		// 前处理
		PicPre_yolov5_hrsid img(img_path, cv::IMREAD_GRAYSCALE);

		img.Resize({ netinfo.i_cubic[0].h, netinfo.i_cubic[0].w }, PicPre_yolov5_hrsid::LONG_SIDE).rPad();
		...
		if (netinfo.DetPost_on) {
			post_detpost_hard(outputs, img,
				netinfo, conf, iou_thresh, MULTILABEL, fpga_nms, N_CLASS, ANCHORS, LABELS,
				show, save, resRoot, name, device, run_sim);
		}
		else {
			post_detpost_soft(outputs, img, LABELS, ANCHORS, netinfo,
				N_CLASS, conf, iou_thresh, show, save, resRoot, name);
		}
