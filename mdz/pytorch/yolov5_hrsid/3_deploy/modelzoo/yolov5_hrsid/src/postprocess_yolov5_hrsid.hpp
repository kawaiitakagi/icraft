...
std::vector<std::vector<float>> coordTrans(std::vector<std::tuple<int, float, cv::Rect2f>>& nms_res, PicPre_yolov5_hrsid& img, bool check_border = true) {
...
void post_detpost_hard(const std::vector<Tensor>& output_tensors, PicPre_yolov5_hrsid& img, NetInfo& netinfo,
...
void post_detpost_soft(const std::vector<Tensor>& output_tensors, PicPre_yolov5_hrsid& img, std::vector<std::string>& LABELS,
        ...
        for (size_t h = 0; h < _H; h++) {
            ...
            for (size_t w = 0; w < _W; w++) {
                ...
                for (size_t anchor_index = 0; anchor_index < ANCHORS.size(); anchor_index++) {
                    ...
                    auto boxPtr = tensor_data + h * _W * (N_CLASS + 5) * ANCHORS.size()
                        + w * (N_CLASS + 5) * ANCHORS.size() + anchor_index * (N_CLASS + 5);
