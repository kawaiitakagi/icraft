...
public:
	...
	PicPre_yolov5_hrsid(const std::string& filename, int flags = IMREAD_COLOR) {
	...
	PicPre_yolov5_hrsid(const cv::Mat& img) {
		this->src_img = img;
		this->src_dims = { src_img.channels(), src_img.rows, src_img.cols };
	}

	//resize，dst_shape_hw:<resized h,resized w>
	PicPre_yolov5_hrsid& Resize(std::pair<int, int> dst_shape_hw, int mode = LONG_SIDE, int interpolation = INTER_LINEAR) {
	...
	*/
	void rPad(int pad_mode = BR) {
		...
		case 0: {
			...
			cv::copyMakeBorder(this->dst_img, this->dst_img, 0, dh, 0, dw, cv::BORDER_CONSTANT, cv::Scalar(144, 144, 144));
			this->_pad_info = { 0,0 };
			break;
		}
		...
		case 1: {
			...
			cv::copyMakeBorder(this->dst_img, this->dst_img, top, bottom, left, right, cv::BORDER_CONSTANT, cv::Scalar(144, 144, 144));
	...
	void rCenterCrop(std::pair<int, int> crop_shape_hw) {
		...
		if (!xxx) {
			std::cerr << "wrong: PicPre_yolov5_hrsid crop shape bigger than inputs " << std::endl;
		}
		...
		this->dst_img = this->dst_img(cv::Rect2f(start_w, start_h, crop_w, crop_h)).clone();
	}

	// return _real_resized_ratio: <ratio_h,ratio_w>
	std::pair<float, float> getRatio() { return _real_resized_ratio; }

	// return _pad_info: <left,top>
	std::pair<int, int> getPad() { return _pad_info; }
