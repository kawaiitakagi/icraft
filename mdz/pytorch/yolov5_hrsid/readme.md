...
- branch：master
- commit_id：e4df1ec
- weights：https://download.fdwxhb.com/data/04_FMSH-100AI/100AI/04_modelzoo/modelzoo_pub/weights/pytorch/yolov5_hrsid/YoloV5s_hrsid.pt

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://download.fdwxhb.com/data/04_FMSH-100AI/100AI/04_modelzoo/modelzoo_pub/weights/pytorch/yolov5_hrsid/YoloV5s_hrsid.pt ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_yolov5_hrsid

```shell
# 在此模型根目录
mkdir 0_yolov5_hrsid
git clone -b master https://github.com/ultralytics/yolov5.git 0_yolov5_hrsid
cd 0_yolov5_hrsid
...
- 0_infer.py                      	    >可以推理一张图并得到最终结果，模型原始权重会从 `/weights	`中寻找，需要您预先下载
  ...
  <div style="background-color: #FFFFCC; color: #000000; padding: 10px; border-left: 5px solid #FFA500;">
  推理使用数据集为校准集内图像，故需先执行最开头的下载脚本，下载量化校准集qtset<br>
  </div>
...
- 1_save.py                              >保存模型，保存好的用于Icraft编译器的模型，会存放在 `/2_compile/fmodel`
  ...
  <div style="background-color: #FFFFCC; color: #000000; padding: 10px; border-left: 5px solid #FFA500;">
  保存模型时的修改点：<br>
1. 模型是由单通道灰度图训练的，故在图片预处理阶段采用单通道处理方式<br>
  2. 将Detect forward中conv算子之后的后处理操作去掉：定义new_forward对forward进行替换 <br>
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/yolov5s_hrsid_deploy_16.toml
     ```
  
      如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）
      
      其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/yolov5_hrsid/imodel
  
# 3. 仿真

通过配置3_deploy/modelzoo/yolov5_hrsid/cfg/中yaml文件的sim字段为True实现模型仿真。
```json
imodel:
   ...
   sim: true
...
1. **编译环境准备**
   ...
   **编译c++程序**
   ...
   ```shell
   #在3.1所需的linux编译环境中
   cd 3_deploy/modelzoo/yolov5_hrsid/build_arm
   cmake ..
   make -j
   ```
...
5. **执行程序**
   ...
   ```
   cd /home/fmsh/ModelZoo/modelzoo/yolov5_hrsid/build_arm
   chmod 777 *
   ./yolov5_hrsid ../cfg/yolov5_hrsid.yaml
   ```
...
| yolov5_hrsid | input shape     | hard time      | qt_strategy     | 精度             |
| -------------- | --------------- | -------------- | --------------- | ---------------- |
| float          | [1, 1, 640, 640] | -              | -               | None |
| int8           | [1, 1, 640, 640] | 5.67248ms | kld-pc | None |
| int16          | [1, 1, 640, 640] | 13.0066ms | kld-pc | None |
