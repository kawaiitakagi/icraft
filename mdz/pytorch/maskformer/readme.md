...
- code：https://github.com/facebookresearch/MaskFormer.git
- branch：main
- commit_id：da3e60d
- weights：https://dl.fbaipublicfiles.com/maskformer/semantic-ade20k/maskformer_R50_bs16_160k/model_final_d8dbeb.pkl

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://dl.fbaipublicfiles.com/maskformer/semantic-ade20k/maskformer_R50_bs16_160k/model_final_d8dbeb.pkl ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_maskformer

```shell
# 在此模型根目录
mkdir 0_maskformer
git clone -b main https://github.com/facebookresearch/MaskFormer.git 0_maskformer
cd 0_maskformer
git checkout da3e60d
```

<div style="background-color: #FFFFCC; color: #000000; padding: 10px; border-left: 5px solid #FFA500;">
注意：在maskformer源码工程运行需要依赖Detectron2，同样，1_script脚本中同样依赖于能跑通maskformer源码工程的python环境，特别注意：本工程中所使用的Detectron2的版本为0.6，0.6版本是经过验证的,其他版本的Detectron2不一定保证正确，最好使用安装0.6版本的Detectron2环境来运行1_script脚本，可参照以下指令安装Detectron2库0.6版本：<br>

   1. git clone -b main https://github.com/facebookresearch/detectron2.git<br>
   2. cd .\detectron2\<br>
   3. git checkout 8c4a333<br>
   4. python -m pip install -e . <br>

也可以安装maskformer源码中的安装说明进行安装，只需要在clone detectron2 git 源码后切换到指定的commitid进行安装即可</div>
...
- 1_save.py                              >保存模型，保存好的用于Icraft编译器的模型，会存放在 `/2_compile/fmodel`
  ...
  1. 固定模型对于输入图片尺寸为640x640 <br>
  2. 将模型前向中实际为定值的tgt和pos_embedding变为模型的属性，从前向中分离 <br>
  3. 更改模型前向中的输入参数类型为tensor,便于trace <br>
      具体修改参见代码
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/maskformer_R50_640x640_8.toml
     ```

     如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）

     其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/maskformer/imodel

# 3. 仿真

通过配置3_deploy/modelzoo/maskformer/cfg/中yaml文件的sim字段为True实现模型仿真。
...
1. **python运行虚拟环境要求与准备**
   ...
   - 在选定的python运行虚拟环境中安装python运行时所需要的依赖包
     ...
     ```powershell
     cd 3_deploy/modelzoo/maskformer
     pip install -r requirements.txt
     #注意：
     #参照以下指令安装Detectron2库0.6版本:
     git clone -b main https://github.com/facebookresearch/detectron2.git
     cd .\detectron2
     git checkout 8c4a333
...
2. **执行程序**
   ...
   ```
   python ./maskformer_psin.py ../cfg/maskformer.yaml
   ```
...
* 测试数据集下载：[ADEChallengeData2016](http://data.csail.mit.edu/places/ADEchallenge/ADEChallengeData2016.zip)
* 下载之后，请参照maskformer源码工程中的datasets文件夹中的readme，运行prepare_ade20k_sem_seg.py ，生成精度测试所需要的标签。

3. **修改 运行时的yaml配置文件**：

运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在 `3_deploy/modelzoo/maskformer/cfg/`

测试时使用：`maskformer_test.yaml`

需要修改：

* imodel中参数：根据所使用板子ip设置url_win，
* 更改自己的dataset精度测试数据集路径。
  * 例如下载好的ADEChallengeData2016，存放路径是“E://maskformer//datasets//ADEChallengeData2016”，则修改为“dir”路径为“E://maskformer//datasets//”
...
```
cd 3_deploy/modelzoo/maskformer/pyrt
python ./maskformer_psin.py ../cfg/maskformer_test.yaml
```

运行完之后，可以在命令窗口看到打印的miou精度信息 也可以在output/log.txt 中查找’‘d2.evaluation.sem_seg_evaluation INFO: OrderedDict(’‘字段后获取。也可以在metrics.log中查询

# 6. 模型性能记录

| maskformer | input shape      | hard time | 精度         |
| --------------- | ---------------- | --------- | ------------ |
|                 | [1,3,640,640] | {'8bit': '248.2826ms', '16bit': '613.4063ms', 'mixed': None} | {'float': ['mIou:43.5824'], 'parse': ['mIou:43.5822'], '8bit': ['kld-pc', 'mIou:41.4750'], '16bit': ['kld-pc', 'mIou:41.8236'], 'mixed': None} |
