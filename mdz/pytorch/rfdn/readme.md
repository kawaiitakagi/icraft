...
- branch：master
- commit_id：b16a6a0
- weights：https://github.com/njulj/RFDN/blob/master/trained_model/RFDN_AIM.pth

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://github.com/njulj/RFDN/blob/master/trained_model/RFDN_AIM.pth ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_rfdn

```shell
# 在此模型根目录
mkdir 0_rfdn
git clone -b master https://github.com/njulj/RFDN 0_rfdn
cd 0_rfdn
git checkout 7cf4e25
```
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/rfdn.toml
     ```

     如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）

     其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/rfdn/imodel

# 3. 仿真

通过配置3_deploy/modelzoo/rfdn/cfg/中yaml文件的sim字段为True实现模型仿真。
...
2. **执行程序**
   ...
   ```
   python ./rfdn_psin.py ../cfg/rfdn.yaml
   ```
...
3. **修改 运行时的yaml配置文件**：
   ...
   运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在 `3_deploy/modelzoo/rfdn/cfg/`

   测试时使用：`rfdn_test.yaml`
...
```
cd 3_deploy/modelzoo/rfdn/pyrt
python ./rfdn_psin.py ../cfg/rfdn.yaml
```
...
```shell
cd 3_deploy/modelzoo/rfdn
mkdir build_win
cd build_win
cmake ..
cmake --build . --config Release
./Release/rfdn_psin.exe ../cfg/rfdn_test.yaml
```

注意：如果使用visualstudio打开sln进行调试，需要右键工程-属性-调试-环境中配置 `./Release/rfdn_psin.exe ../cfg/rfdn_test.yaml`
...
| rfdn            | input shape      | hard time       | qt_strategy      | 精度             |
| --------------- | ---------------- | --------------- | ---------------- | -----------------|
| float           | [1, 3, 160, 240] | -               | -                | 31.891           |
| int8            | [1, 3, 160, 240] | 12.42ms         | null-pt          | 30.839           |
| int16           | [1, 3, 160, 240] | 23.63ms         | null-pt          | 31.889           |
| mix             | [1, 3, 160, 240] | 16.54ms         | null-pt          | 30.876           |
