...
def infer(self, setting, test=0):
    ...
    with torch.no_grad():
        if self.args.task_name == 'long_term_forecast':
            self.model = torch.jit.load(self.args.model_pt)
            ...
            for i, (batch_x, label, padding_mask) in enumerate(test_loader):
                ...
                # traced_script_module = torch.jit.trace(self.model.cpu(),(batch_x.cpu(), pos_emb.cpu(), hx.cpu()))
                traced_out = self.model(batch_x.cpu())
                outputs = traced_out
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_pt', type=str, default=R'../2_compile/fmodel/segrnn_ltf.pt', help='torchscript model path')
