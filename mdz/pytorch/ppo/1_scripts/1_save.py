...
class CustomPPO(nn.Module):
    ...
    def forward(
        ...
        return self.base_model.get_distribution(observation).get_actions(deterministic=self.deterministic)
...
def enjoy() -> None:  # noqa: C901
    ...
    model_trace = CustomPPO(model,deterministic=deterministic)
    try:
        generator = range(1)
        model_trace.eval()
        save_path = R"..\3_deploy\modelzoo\ppo\imodel"
        for _ in generator:
            with th.no_grad():
                obs_trace =  deepcopy(obs).astype(np.float32)
                obs_trace, vectorized_env = model_trace.base_model.obs_to_tensor(obs_trace) 
                th.onnx.export(model_trace, obs_trace, save_path+"\\ppo_carracing.onnx", verbose=True)
                print('Onnx model export success, saved in %s' % save_path)

                ftmp_path = R"..\3_deploy\modelzoo\ppo\io\inputs"
