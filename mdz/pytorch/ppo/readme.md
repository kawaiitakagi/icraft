...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_ppo
...
- 1_save.py     >保存模型与ONNX输入，保存为onnx模型，会存放在 `\3_deploy\modelzoo\dqn\imodel`，输入放在`\3_deploy\modelzoo\dqn\io`
...
2. **版本依赖下载**
   ...
   主要版本依赖已存放在`\3_deploy\modelzoo\ppo\onnxruntime`
...
3. **编译c++程序**
   ...
   ```shell
   #在3.1所需的linux编译环境中
   cd 3_deploy/modelzoo/ppo/build_arm
   ...
   模型输入均在 `3_deploy/modelzoo/ppo/io`中，可根据需要进行替换，生成方式如下：
   
   ```
   # 仅测试时间，python中随机生成输出
   obs_onnx = torch.randn(1,2,64,64,dtype=torch.float32)
   obs_onnx.tofile("encoder_input.ftmp")
   # 最后手动放入对应3_deploy/modelzoo_v3.0.1/ppo/io/inputs/ppo目录中
   ```
...
5. **执行程序**
   ...
   ```
   cd /home/fmsh/ModelZoo/modelzoo/ppo/build_arm
   chmod 777 *
   ./ort_run
   ```
