...
def new_export_torchscript(self, prefix=colorstr('TorchScript:')):
    ...
    # plin
    pl_im = torch.randn(1,3,512,960,dtype=torch.float32)
    pl_ts = torch.jit.trace(self.model.cpu(), pl_im, strict=False)
    pl_ts.save("../2_compile/fmodel/yolov8n_512x960.pt")
