...
- code：https://github.com/maudzung/SFA3D
- master
- 0e2f0b6
- weights：https://github.com/maudzung/SFA3D/blob/master/checkpoints/fpn_resnet_18/fpn_resnet_18_epoch_300.pth

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://github.com/maudzung/SFA3D/blob/master/checkpoints/fpn_resnet_18/fpn_resnet_18_epoch_300.pth ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_SFA3D-master

```shell
# 在此模型根目录
mkdir 0_SFA3D-master
git clone -b master https://github.com/maudzung/SFA3D.git 0_SFA3D-master
cd 0_SFA3D-master
git checkout 0e2f0b6
```
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/sfa3d.toml
     ```
...
通过配置3_deploy/modelzoo/TarDAL/cfg/中yaml文件的sim字段为True实现模型仿真。
```json
imodel:
   ...
   sim: true
...
2. **执行程序**
   ...
   ```
   python ./SFA3D_psin.py ../cfg/SFA3D.yaml
   ```
...
| SFA3D | input shape     | hard time      | qt_strategy     | 精度             |
| -------------- | --------------- | -------------- | --------------- | ---------------- |
| float          | [1, 3, 608, 608] | -              | -               | Car: 3d: Easy_ap50:87.05; Moderate_ap50:83.24, Hard_ap50:85.20 |
| 8bit           | [1, 3, 608, 608] | 198.1865ms  | kld-pc | Car: 3d: Easy_ap50:75.65; Moderate_ap50:74.43, Hard_ap50:78.28 |
| 16bit          | [1, 3, 608, 608] | 219.5726ms | kld-pc | Car: 3d: Easy_ap50:87.16; Moderate_ap50:83.31, Hard_ap50:85.26 |
