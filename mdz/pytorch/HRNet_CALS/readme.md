...
- 0_HRNet_CALS    >模型原始工程，需要自行下载(CALS - Carrier Assisted Landing System)
...
- code：https://gitee.com/mxh-spiger/custom_model/tree/master/HRNet
- branch：master
- commit_id：a8ca2c4
- weights：https://download.fdwxhb.com/data/04_FMSH-100AI/100AI/04_modelzoo/modelzoo_pub/weights/pytorch/HRNet_CALS/model_best.pth.tar
...
1）根据模型来源中的地址：   
  https://download.fdwxhb.com/data/04_FMSH-100AI/100AI/04_modelzoo/modelzoo_pub/weights/pytorch/HRNet_CALS/model_best.pth.tar   
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_HRNet_CALS

```shell
# 在此模型根目录
mkdir 0_HRNet_CALS
git clone -b master https://gitee.com/mxh-spiger/custom_model.git 0_HRNet
cd 0_HRNet
git checkout 80f2e92
```
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/HRNet_8.toml
     ```
  
      如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）
  
      其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/HRNet_CALS/imodel
  
# 3. 仿真

通过配置3_deploy/modelzoo/HRNet_CALS/cfg/中yaml文件的sim字段为True实现模型仿真。
```json
imodel:
   ...
   sim: true
...
**编译c++程序**
   ...
   ```shell
   #在3.1所需的linux编译环境中
   cd 3_deploy/modelzoo/HRNet_CALS/build_arm
   cmake ..
   make -j
   ```
...
5. **执行程序**
   ...
   ```
   cd /home/fmsh/ModelZoo/modelzoo/HRNet_CALS/build_arm
   chmod 777 *
   ./HRNet_CALS ../cfg/HRNet_CALS.yaml
   ```
...
本工程还提供了两种PLIN demo
### 单线程工程示例
`HRNet_CALS_plin.cpp`：为HRNet_CALS关键点检测的单线程PLIN demo<br>
使用方法：
* 进入`2_compile` 重新编译模型，编译配置为：`HRNet_8_plin.toml`
* 进入`3_deploy` 修改CMakeList.txt
```C++
# set sln name
set(TARGET_NAME HRNet_plin)
```
...
```
### 多线程工程示例
`HRNet_CALS_plin_task_queue.cpp`：为HRNet_CALS关键点检测的多线程PLIN demo<br>
使用方法：
* 进入`2_compile` 重新编译模型，编译配置为：`HRNet_8_plin.toml`
* 进入`3_deploy` 修改CMakeList.txt
```C++
# set sln name
set(TARGET_NAME HRNet_CALS_plin_task_queue)
```
* 编译c++程序，将执行文件拷贝到板子上，程序执行指令如下
```
cd /home/fmsh/ModelZoo/modelzoo/HRNet_CALS/build_arm
chmod 777 *
./HRNet_CALS_plin_task_queue ../cfg/HRNet_CALS.yaml
```
...
* 测试数据集下载：[DCS](https://github.com/DCS-Skunkworks/dcs-bios)

* 测试图片存放于  `3_deploy\modelzoo\HRNet_CALS\io\input`及`3_deploy\modelzoo\HRNet_CALS\io\input.txt`中
* 如需测试其他数据集，需自行将图片存放于`3_deploy\modelzoo\HRNet_CALS\io\input`,并准备对应的input.txt



3. **修改 运行时的yaml配置文件**：

   运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在`3_deploy/modelzoo/HRNet_CALS/cfg/`

   测试时使用：`HRNet_CALS_test.yaml`
...
```shell
cd 3_deploy/modelzoo/HRNet_CALS
mkdir build_win
cd build_win
cmake ..
cmake --build . --config Release
./Release/HRNet_CALS.exe ../cfg/HRNet_CALS_test.yaml
```

注意：如果使用visualstudio打开sln进行调试，需要右键工程-属性-调试-环境中配置`./Release/HRNet_CALS.exe ../cfg/HRNet_CALS_test.yaml`
...
| HRNet | input shape     | hard time      | qt_strategy     | 精度(pixel-error)             |
| -------------- | --------------- | -------------- | --------------- | ---------------- |
| float          | [1, 3, 640, 640] | -              | -               | 2.22 |
| 8bit           | [1, 3, 640, 640] | 23.427 ms  | null-pt | 2.901 |
| 16bit          | [1, 3, 640, 640] | 51.365 ms | null-pt | 2.804 |
