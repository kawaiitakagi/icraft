...
- code：https://github.com/jfzhang95/pytorch-deeplab-xception.git
- branch：master
- commit_id：9135e10
- weights：https://drive.google.com/file/d/1NwcwlWqA-0HqAPk3dSNNPipGMF0iS0Zu/view

## 2. **保存模型**

**目的：将模型保存成可以被Icraft编译器编译的形态**

1）根据模型来源中的地址：https://drive.google.com/file/d/1NwcwlWqA-0HqAPk3dSNNPipGMF0iS0Zu/view ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_deeplabv3plus

```shell
# 在此模型根目录
mkdir 0_deeplabv3plus
git clone -b master https://github.com/jfzhang95/pytorch-deeplab-xception.git 0_deeplabv3plus
cd 0_deeplabv3plus
git checkout 9135e10
```
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/deeplabv3plus_8.toml
     ```
  
      如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）
      
      其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/deeplabv3plus/imodel
  
# 3. 仿真

通过配置3_deploy/modelzoo/deeplabv3plus/cfg/中yaml文件的sim字段为True实现模型仿真。
```json
imodel:
   ...
   sim: true
...
1. **编译环境准备**
   ...
   **编译c++程序**
   ...
   ```shell
   #在3.1所需的linux编译环境中
   cd 3_deploy/modelzoo/deeplabv3plus/build_arm
   cmake ..
   make -j
   ```
...
5. **执行程序**
   ...
   ```
   cd /home/fmsh/ModelZoo/modelzoo/deeplabv3plus/build_arm
   chmod 777 *
   ./deeplabv3plus_psin ../cfg/deeplabv3plus.yaml
   ```
...
1. **数据集准备**
   ...
   测试数据集下载：[voc2012val](http://pjreddie.com/media/files/VOCtrainval_11-May-2012.tar)

   * 模型测试使用voc2012数据集的Train部分中的Val集合图片，下载完数据集后将JPEGImages中图片存放于  `3_deploy\modelzoo\deeplabv3plus\io\input`中，val的图片集合在下载后数据集的`ImageSets\Segmentation\val.txt` 中，需要将其复制到`3_deploy\modelzoo\deeplabv3plus\io\`中
   * 如需测试其他数据集，需自行将图片存放于`3_deploy\modelzoo\deeplabv3plus\io\input`,并准备对应的input.txt



2. **修改 运行时的yaml配置文件**：

   运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在`3_deploy/modelzoo/deeplabv3plus/cfg/`

   测试时使用：`deeplabv3plus_test.yaml`

   需要修改：

   * imodel中参数：设置imodel路径，并根据所使用板子ip设置ip，如需保存图片将save设置为true，如需可视化将show设置为true，反之，设置为false；
   * dataset中参数：设置dataset精度测试数据集路径；
...
3. **编译、执行运行时程序**
   ...
   ```shell
   cd 3_deploy/modelzoo/deeplabv3plus
   mkdir build_win
   cd build_win
   cmake ..
   cmake --build . --config Release
   ./Release/deeplabv3plus_psin.exe ../cfg/deeplabv3plus_test.yaml
   ```

   注意：如果使用visualstudio打开sln进行调试，需要右键工程-属性-调试-命令参数中配置`../cfg/deeplabv3plus_test.yaml`
...
4. **使用metrics测试脚本，将保存的测试结果计算出精度指标**
   ...
   - 使用说明:[semantic-seg_voc2012/readme.md · spiger/benchmark - 码云 - 开源中国 (gitee.com)](https://gitee.com/mxh-spiger/benchmark/blob/master/semantic-seg_voc2012/readme.md)



# 6. 模型性能记录

| deeplabv3plus | input shape     | hard time      | qt_strategy     | 精度             |
| -------------- | --------------- | -------------- | --------------- | ---------------- |
| float          | [1, 3, 513, 513] | -              | -               | mIoU:0.784 |
| 8bit           | [1, 3, 513, 513] | 51.606ms | kld-pc | mIoU:0.766 |
| 16bit          | [1, 3, 513, 513] | 209.6435ms | kld-pt | mIoU:0.769 |
