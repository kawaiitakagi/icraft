...
#include "postprocess_ppyoloe.hpp"
...
int main(int argc, char* argv[])
{
	...
	for (auto name : namevector) {
		...
		if (netinfo.DetPost_on) {
			...
			std::vector<std::vector<float>> _norm = set_norm_by_head(NOH, parts, normalratio);
    		std::vector<float> _stride = get_stride(netinfo);
			post_detpost_hard(outputs, img,netinfo, conf, iou_thresh, MULTILABEL, fpga_nms, N_CLASS,ANCHORS,LABELS,show,save,resRoot,name,device,run_sim,_norm,real_out_channles,_stride,bbox_info_channel);
		}
		else{
			post_detpost_soft(outputs, img, LABELS, ANCHORS, netinfo,N_CLASS, conf, iou_thresh,fpga_nms, device, run_sim, MULTILABEL,show, save, resRoot, name);
		}
