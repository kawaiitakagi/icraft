...
- code：https://github.com/PaddlePaddle/PaddleYOLO.git
- branch：release/2.6
- commit_id：89e3ff8
...
1）根据模型来源中的地址：https://paddledet.bj.bcebos.com/models/yolov5_s_300e_coco.pdparams ，下载原始weights，存放于 `/weights`文件夹中
...
2）根据模型来源中的地址，下载指定commit id版本的源代码，文件夹名称要设置为：0_ppyolov5

```shell
# 在此模型根目录
mkdir 0_ppyolov5
git clone -b release/2.6 https://github.com/PaddlePaddle/PaddleYOLO.git 0_ppyolov5
cd 0_ppyolov5
git checkout 89e3ff8
```
...
- 1_save.py                              >保存模型，保存好的用于Icraft编译器的模型，会存放在 `/2_compile/fmodel`
  ...
  1. 导出到卷积计算为止，固定输入尺寸1，3，640，640<br>
     </div>
...
- 3）**执行编译：**
  ...
  **在 `/2_compile`目录下执行编译：**
     ...
     ```shell
      icraft compile config/ppyolov5s_8.toml
     ```
  
      如果过程顺利，将得到 icraft model（以 `.json` （graph）`.raw`（param）的格式保存）
  
      其中包括编译各阶段产生的中间结果模型和最终用于片上部署的BY模型，直接被保存到:  3_deploy/modelzoo/ppyolov5/imodel
  
# 3. 仿真

通过配置3_deploy/modelzoo/ppyolov5/cfg/中yaml文件的sim字段为True实现模型仿真。
```json
imodel:
   ...
   sim: true
...
**编译c++程序**
   ...
   ```shell
   #在3.1所需的linux编译环境中
   cd 3_deploy/modelzoo/ppyolov5/build_arm
   cmake ..
   make -j
   ```
...
5. **执行程序**
   ...
   ```
   cd /home/fmsh/ModelZoo/modelzoo/ppyolov5/build_arm
   chmod 777 *
   ./ppyolov5s_psin ../cfg/ppyolov5s.yaml
   ```
...
3. **修改 运行时的yaml配置文件**：
   ...
   运行时需要配置一些模型路径，输入数据，后处理参数等，其配置文件在`3_deploy/modelzoo/ppyolov5/cfg/`

   测试时使用：`ppyolov5s_test.yaml`
...
4. **编译、执行运行时程序**
   ...
   ```shell
   cd 3_deploy/modelzoo/ppyolov5
   mkdir build_win
   cd build_win
   cmake ..
   cmake --build . --config Release
   ./Release/ppyolov5s_psin.exe ../cfg/ppyolov5s_test.yaml
   ```

   注意：如果使用visualstudio打开sln进行调试，需要右键工程-属性-调试-环境中配置`./Release/ppyolov5s_psin.exe ../cfg/ppyolov5s_test.yaml`
...
| ppyolov5 | input shape     | hard time      | qt_strategy     | 精度             |
| -------------- | --------------- | -------------- | --------------- | ---------------- |
| float          | [1, 3, 640, 640] | -              | -               | ap50:0.568 |
| int8           | [1, 3, 640, 640] | 5.78ms | kld-pc | ap50:0.559; ap50-95:0.345 |
| int16          | [1, 3, 640, 640] | 13.32ms | kld-pc | ap50:0.567; ap50-95:0.365 |
